...
class SimpleFieldsUrlCombo extends SimpleFieldsCombo {
   ...
   */
  get sortedOptions() {
    let sorted = (this.itemsList || []).map((item, i) => typeof item === "object" ? item : this.getOptionData(item, i));
    Object.keys(this.options || {}).sort((a, b) => a > b ? 1 : -1).forEach(key => sorted.push(this.getOptionData(this.options[key], sorted.length)));
    return sorted;
  }
  /**
  ...
  filterOptions(filter, currentOption) {
    ...
    if (this.possibleEmail(filter)) {
      ...
      this.filteredOptions = [{
        value: `tel:${filter.replace(/[\(\)\-\s\:\.]/g, "").replace(/[a-zA-Z]+/, "p")}`,
        id: "tel"
      }, ...this.filteredOptions];
      this.open();
    }
