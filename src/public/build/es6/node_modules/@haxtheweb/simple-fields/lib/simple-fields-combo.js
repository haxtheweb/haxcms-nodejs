...
class SimpleFieldsCombo extends SimpleFieldsFieldBehaviors(LitElement) {
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (propName === "value" && this.value !== oldValue) this.fieldValueChanged();
      if (propName === "itemsList" || propName === "options") this.filterOptions(this.filter, this.option);
    });
  }
  fieldValueChanged() {
  ...
  get listboxTemplate() {
    return html`
      <ul
        ...
        data-options="${this.filteredOptions.map(option => option.value).join()}"
  ...
  get listboxInnerTemplate() {
    return (this.filteredOptions || []).map(option => this.getListItem(option));
  }
  ...
  setOption(option, flag = false) {
    if (option) {
      ...
      if (this.isBoth) {
        ...
        if (flag) {
          setTimeout(() => {
            this.input.setSelectionRange(option.value.length, option.value.length);
          }, 0);
        } else {
          setTimeout(() => {
            this.input.setSelectionRange(this.filter.length, option.value.length);
          }, 0);
        }
      }
    }
  ...
  setCurrentOptionStyle(option) {
    ...
    if (!!this.listbox && !!option && !!this.shadowRoot && !!this.shadowRoot.querySelector(`#option${option.id}`)) this.listbox.scrollTop = this.shadowRoot.querySelector(`#option${option.id}`).offsetTop;
  }
  _isSelected(option) {
  ...
  get listbox() {
    if (!this._listbox) this._listbox = this.shadowRoot && this.shadowRoot.querySelector(`#${this.id}-list`) ? this.shadowRoot.querySelector(`#${this.id}-list`) : undefined;
    return this._listbox;
  }
  get input() {
    if (!this._input) this._input = this.field ? this.field : this.shadowRoot && this.shadowRoot.querySelector(`#${this.id}`) ? this.shadowRoot.querySelector(`#${this.id}`) : undefined;
    return this._input;
  }
  get keyCode() {
    return {
