...
export class SuperDaemonUI extends SimpleFilterMixin(I18NMixin(SimpleColors)) {
  constructor() {
    ...
    autorun(() => {
      ...
      if (UserScaffoldInstance.active && UserScaffoldInstance.memory.isLoggedIn && sdi.programName === null && usAction.type === "drag") {
        this.activeDrag = true;
        this.activeType = usData.value || usData.architype;
      } else if (UserScaffoldInstance.active && UserScaffoldInstance.memory.isLoggedIn && sdi.programName === null && usAction.type === "dragleave") {
        this.activeDrag = false;
        this.activeType = null;
      }
    });
      ...
      switch (e.key) {
        ...
        case "ArrowLeft":
          ...
          if (this.shadowRoot.querySelector("super-daemon-row[active]") === this.shadowRoot.querySelector("super-daemon-row")) {
            this.shadowRoot.querySelector("lit-virtualizer").scrollToIndex(this.filtered.length - 1, "center");
            requestAnimationFrame(() => {
              this.shadowRoot.querySelector("super-daemon-row:last-of-type").focus();
            });
          } else {
            this.shadowRoot.querySelector("super-daemon-row[active]").previousElementSibling.focus();
          }
          break;
        case "ArrowDown":
        case "ArrowRight":
          // allow wrap around
          if (this.shadowRoot.querySelector("super-daemon-row[active]") === this.shadowRoot.querySelector("super-daemon-row:last-of-type")) {
            this.shadowRoot.querySelector("lit-virtualizer").scrollToIndex(0, "center");
            requestAnimationFrame(() => {
              this.shadowRoot.querySelector("super-daemon-row").focus();
            });
          } else {
            this.shadowRoot.querySelector("super-daemon-row[active]").nextElementSibling.focus();
          }
          break;
      }
    }
    ...
    return html`
      ...
      >
        ${this.loading ? html`<div class="loading">${this.t.loadingResults}..</div>` : html`
              ${!this.filtered.length || this.filtered.length === 0 ? html`<div class="no-results">
