...
import { mimeTypeToName } from "../../utils/utils.js";
...
export class SuperDaemonSearch extends I18NMixin(SimpleColors) {
      ...
      switch (e.key) {
        ...
        case "Backspace":
          ...
          if (this.programSearch == "" && this.programName) {
            ...
            e.preventDefault();
          } else if (!this.programName && this.value == "" && this.commandContext) {
            this.commandContext = "*";
            e.preventDefault();
          }
          break;
  ...
  render() {
    return html` ${this.commandContext != "*" ? html`<simple-icon-lite
      ...
      <simple-fields-field
        ...
        part="filter"
      ></simple-fields-field>
      ${this.voiceSearch ? html`<simple-icon-button-lite
  ...
  firstUpdated(changedProperties) {
    ...
    this.shadowRoot.querySelector("#inputfilter").placeholder = this.suggestPossibleAction();
  }
  updated(changedProperties) {
    ...
    if (changedProperties.has("wand")) {
      if (!this.wand) {
        ...
        if (sdi.opened) {
          this.possibleActions = ["🧑 Submit your ideas💡", "📁 Drop files here 📄", "🕵 Type what you want to do"];
        } else {
          this.possibleActions = [`🧙‍♂️ ${sdi.key1} + ${sdi.key2} opens Merlin`, "🔮 Click to do anything!", "📁 Drop files here 📄"];
        }
      }
    }
    if (changedProperties.has("droppableType") && this.shadowRoot) {
      this.shadowRoot.querySelector("#inputfilter").placeholder = this.suggestPossibleAction(this.droppableType);
    }
