...
import { SimpleToastEl } from "../../simple-toast/lib/simple-toast-el.js";
import "../../future-terminal-text/future-terminal-text.js";
...
export class SuperDaemonToast extends SimpleToastEl {
  ...
  render() {
    return html` <div class="bubble">
      ${this.awaitingMerlinInput ? html`<simple-icon-lite
            class="awaiting-input"
            icon="hax:loading"
          ></simple-icon-lite>` : html`
            <simple-icon
              class="merlin"
              icon="hax:wizard-hat"
              accent-color="purple"
      ...
      <span class="bubble mid">
        <slot name="pre"></slot>
        ${this.future ? html` <future-terminal-text
              fadein
              glitch
              glitch-max="3"
              glitch-duration="40"
            ></future-terminal-text>` : html`${this.text}`}
        <slot></slot>
      </span>
  ...
  connectedCallback() {
    ...
    globalThis.addEventListener("super-daemon-toast-hide", this.hideSimpleToast.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("super-daemon-toast-show", this.showSimpleToast.bind(this), {
      signal: this.windowControllers.signal
    });
  }
  ...
  show() {
    if (!this.opened) {
      this.style.animation = "fadein 0.3s, fadeout 0.6s " + this.duration / 1000 + "s";
      this.opened = true;
    }
  }
  hide() {
...
globalThis.SuperDaemonToast.requestAvailability = () => {
  if (!globalThis.SuperDaemonToast.instance) {
    globalThis.SuperDaemonToast.instance = globalThis.document.createElement(SuperDaemonToast.tag);
    globalThis.document.body.appendChild(globalThis.SuperDaemonToast.instance);
  }
  return globalThis.SuperDaemonToast.instance;
};
export const SuperDaemonToastInstance = globalThis.SuperDaemonToast.requestAvailability();