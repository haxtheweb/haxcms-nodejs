...
class SimpleAutocomplete extends SimpleFilterMixin(LitElement) {
  ...
  render() {
    return html`
      ...
      <simple-popover
        ...
        for="input"
      >
        ${this.filtered.length > 0 && this.opened ? html` <ul
              part="list"
              role="listbox"
              @keydown="${this.a11yListKeys}"
            >
              ${this.filtered.map((item, i) => html`${i < this.itemLimit ? html`<li
                        role="option"
                        part="list-item"
                        value="${item.value}"
                        data-index="${i}"
            ...
            </ul>` : html`<div class="no-results" part="no-result">No results</div>`}
      </simple-popover>
    `;
  }
  /**
  ...
  getSelection() {
    ...
    if (this.$input) {
      ...
      else if (typeof this.$input.endOffset === "undefined" && getRange(this.$input)) {
