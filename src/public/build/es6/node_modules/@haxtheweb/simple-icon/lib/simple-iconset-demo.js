 ...
 */
import { html, svg, css, LitElement } from "../../../lit/index.js";
...
class SimpleIconsetDemo extends LitElement {
  ...
  // render function
  render() {
    return this.iconsets.length < 1 ? "Looking for iconsets..." : this.iconsets.map(iconset => html`<div class="iconset">
  ...
  _getIconsets() {
    let imports = this.imports.length < 1 ? [SimpleIconIconsetsManifest, HaxIconsetManifest] : this.imports,
      iconsets = imports.flat(),
      excludeSets = (this.exclude || "").split(" "),
      includeSets = (this.include || "").split(" ");
    this.iconsets = iconsets.filter(iconset => !!iconset && !excludeSets.includes(iconset) && (!this.include || includeSets.includes(iconset)));
  }
}
