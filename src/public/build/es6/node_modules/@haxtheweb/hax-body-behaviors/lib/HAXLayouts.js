  ...
  return class extends SchemaBehaviors(SuperClass) {
        ...
        if (propName === "dataHaxRay" && this.shadowRoot) {
          if (this[propName]) {
            ...
            containers.forEach(container => {
              ...
              container.addEventListener("dragleave", this._dragLeave.bind(this));
            });
            slots.forEach(slot => slot.addEventListener("slotchange", this._slotMonitor.bind(this)));
                ...
                mutations.forEach(mutation => {
                  ...
                  mutation.addedNodes.forEach(node => {
                    if (node.tagName && node !== this) {
                      ...
                      if (node.parentElement && node.parentElement.tagName !== "HAX-BODY" && !this.validateElementSlot(node) && this.validElementSlots().length > 0) {
            ...
            let containers = [...this.shadowRoot.querySelectorAll("drag-enabled")],
              slots = [...this.shadowRoot.querySelectorAll("slot")];
            containers.forEach(container => {
              container.removeEventListener("dragenter", this._dragEnter.bind(this));
              container.removeEventListener("dragleave", this._dragLeave.bind(this));
            });
            slots.forEach(slot => slot.removeEventListener("slotchange", this._slotMonitor.bind(this)));
          }
        }
    ...
    canMoveSlot(item, before) {
      let dir = before ? -1 : 1,
        ...
        containers = [...this.shadowRoot.querySelectorAll("[data-layout-order]")].map(container => parseInt(container.getAttribute("data-layout-order") || -1)).sort((a, b) => a - b),
        dest = order[1] + dir;
    ...
    moveSlot(item, before) {
      let dir = before ? -1 : 1,
        order = this.this._getSlotOrder(item),
        dest = order[1] + dir,
        container = this.shadowRoot.querySelector(`[data-layout-order=${dest}]`),
    ...
    validElementSlots() {
      return this.shadowRoot ? [...this.shadowRoot.querySelectorAll("[data-layout-order]")].map(container => container.getAttribute("data-layout-slotname")) : [];
    }
    /**
