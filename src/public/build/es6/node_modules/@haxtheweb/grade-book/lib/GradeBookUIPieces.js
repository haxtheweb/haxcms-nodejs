  ...
  return class extends SuperClass {
    ...
    renderActiveStudentBtn() {
      return html`<simple-icon-button-lite
          ...
          ?disabled="${!this.ready}"
        >
          ${this.database.roster[this.activeStudent] && this.database.roster[this.activeStudent].photo ? html`<img
        ...
        >
          <div>
            ${this.database.roster[this.activeStudent] ? html` <grade-book-student-block
                  .student="${this.database.roster[this.activeStudent]}"
                ></grade-book-student-block>` : nothing}

            <div class="active-student-grade-history">
              ${this.activeStudentSubmissions.map(sRecord => html`
                  <button
                    .value="${sRecord.assignmentIndex}"
                    @click="${this.studentLetterGradeHistoryClick}"
                    class="${this.activeAssignment === sRecord.assignmentIndex ? `activeAssignment` : ``}"
                  >
                    <letter-grade
                      mini
          ...
          >
            <table>
              <tbody>
                ...
                ${this.database.gradeScale.map(scale => html`
                    <tr>
                      <td>${scale.letter}</td>
    ...
    renderActiveAssignmentBtn() {
      return html`<simple-icon-button-lite
        ...
        <simple-popover
          ...
          auto
        >
          ${this.database.assignments[this.activeAssignment] ? html`
                <h3>
    ...
    renderDisplayModeBtn() {
      return html` ${this.displayModeData().map((value, i) => html`
          <simple-icon-button-lite
            icon="${value.icon}"
