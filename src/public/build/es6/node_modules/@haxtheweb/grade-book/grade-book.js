 ...
 */
import { html, css, render, nothing } from "../../lit/index.js";
import { SimpleColors } from "../simple-colors/simple-colors.js";
import { I18NMixin } from "../i18n-manager/lib/I18NMixin.js";
import { validURL, cleanVideoSource, isElementInViewport } from "../utils/utils.js";
import { gSheetInterface } from "../utils/lib/gSheetsInterface.js";
import { normalizeEventPath } from "../utils/utils.js";
import "../simple-fields/lib/simple-fields-field.js";
import "../simple-fields/lib/simple-fields-tag-list.js";
import "../a11y-collapse/a11y-collapse.js";
import "../a11y-collapse/lib/a11y-collapse-group.js";
import "../editable-table/lib/editable-table-display.js";
import "../a11y-tabs/a11y-tabs.js";
import "../a11y-tabs/lib/a11y-tab.js";
import "../grid-plate/grid-plate.js";
import "../iframe-loader/lib/loading-indicator.js";
import "./lib/letter-grade.js";
import "./lib/letter-grade-picker.js";
import "../../@github/time-elements/dist/index.js";
...
import { ESGlobalBridgeStore } from "../es-global-bridge/es-global-bridge.js";
import { XLSXFileSystemBrokerSingleton } from "../file-system-broker/lib/xlsx-file-system-broker.js";
/**
 * `grade-book`
...
class GradeBook extends UIRenderPieces(I18NMixin(SimpleColors)) {
  constructor() {
    ...
    this.addEventListener("simple-fields-tag-list-changed", this.qualitativeFeedbackUpdate.bind(this));
      ...
      this.__resizer = setTimeout(() => {
        ...
        for (let entry of entries) {
          if (entry.contentBoxSize) {
            pixels = Math.round(globalThis.innerHeight - entry.contentBoxSize[0].blockSize - 122);
          } else {
            pixels = Math.round(globalThis.innerHeight - entry.contentRect.height - 122);
          }
        }
        this.shadowRoot.querySelector("#studentassessment").shadowRoot.querySelector("[part='content']").style.height = pixels + "px";
      }, 50);
    });
  ...
  getActiveRubric() {
    return this.database.rubrics.filter(item => {
      return item.shortName == this.database.assignments[this.activeAssignment].rubric;
    });
  }
  /**
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (!this.loading && ["activeAssignment", "activeStudent", "database", "loading"].includes(propName)) {
        setTimeout(() => {
          // this will defer to whatever the "grades" db value is
          this.totalScore = this.getCurrentScore(this.activeStudent, this.activeAssignment);
          this.assessmentView = this.resetAssessmentView();
          this.activeRubric = [...this.getActiveRubric()];
          this.hideRubricInfo = [...this.activeRubric.map(() => {
            return false;
          })];
          this.activeStudentSubmissions = [];
          this.activeStudentSubmissions = [...this.getStudentSubmissions(this.activeStudent)];
      ...
      if (["sourceData", "source"].includes(propName)) {
        if (this.sourceData) {
          switch (this.source) {
            case "googledocs":
              ...
              setTimeout(async () => {
                for (var i in this.gSheet.sheetGids) {
                  ...
                  if (i === "gradeScale" && this.database.gradeScale && this.database.gradeScale.length > 0) {
            ...
            case "url":
              this.loading = true;
              fetch(this.sourceData).then(response => {
                if (response.ok) {
  ...
  maintainScrollPosition() {
    ...
    if (this.shadowRoot && this.shadowRoot.querySelector("#studentgrid [data-active]")) {
      setTimeout(() => {
        let offset = 192;
        let isVisible = isElementInViewport(this.shadowRoot.querySelector("#studentgrid [data-active]"), {
          top: 0,
          right: globalThis.innerWidth,
          bottom: this.shadowRoot.querySelector("#studentgrid").getBoundingClientRect().height + 20,
          left: offset
        });
        if (GradeBookStore.activeStudent === 0) {
          this.shadowRoot.querySelector("#studentgrid").scrollTop = 0;
        } else if (!isVisible) {
          this.shadowRoot.querySelector("#studentgrid").scrollTop = this.shadowRoot.querySelector("#studentgrid [data-active]").offsetTop - this.shadowRoot.querySelector("#studentgrid").offsetTop;
        }
        // left to right
        if (GradeBookStore.activeAssignment === 0) {
          this.shadowRoot.querySelector("#studentgrid").scrollLeft = 0;
        } else if (!isVisible) {
          this.shadowRoot.querySelector("#studentgrid").scrollLeft = this.shadowRoot.querySelector("#studentgrid [data-active]").offsetLeft - offset - this.shadowRoot.querySelector("#studentgrid").offsetLeft;
        }
      }, 0);
    }
  }
  ...
  async changeAssignment(e) {
    ...
    } else if (e.target.getAttribute("value") == "next" && this.database.assignments.length - 1 !== GradeBookStore.activeAssignment) {
      GradeBookStore.activeAssignment++;
    }
    await this.requestUpdate();
  }
  ...
  renderSubmission(data) {
    const height = parseInt(this.shadowRoot.querySelector("#studentgrid").getBoundingClientRect().height);
    this.shadowRoot.querySelector("#studentassessment").shadowRoot.querySelector("[part='content']").style.height = Math.round(globalThis.innerHeight - height - 104) + "px";
  ...
  activateOption(e) {
    ...
    if (target[0].getAttribute("data-student") && target[0].getAttribute("data-assignment")) {
      GradeBookStore.activeAssignment = parseInt(target[0].getAttribute("data-assignment"));
      GradeBookStore.activeStudent = parseInt(target[0].getAttribute("data-student"));
  ...
  mouseHighlight(e) {
    ...
    this.__mdebounce = setTimeout(() => {
      if (active && active.getAttribute("data-assignment") != this.__activeHoverAssignment) {
        this.shadowRoot.querySelectorAll('#studentgrid .th-or-td[data-assignment="' + this.__activeHoverAssignment + '"]').forEach(el => {
          el.classList.remove("col-highlight");
        });
  ...
  studentreportClick(e) {
    ESGlobalBridgeStore.import("jspdf", new URL(`./lib/grade-book.js`, import.meta.url).href + "/../jspdf.min.js").then(() => {
      var pdf = new jsPDF();
      pdf.fromHTML(this.shadowRoot.querySelector("#studentreport").outerHTML, 15, 15);
      const cd = new Date();
      const dateTime = `${cd.getFullYear()}-${cd.getMonth()}-${cd.getDate()}__${cd.getHours()}-${cd.getMinutes()}-${cd.getSeconds()}`;
      const fname = `${this.database.roster[this.activeStudent].student}--${this.database.assignments[this.activeAssignment].shortName}--${dateTime}.pdf`;
      pdf.save(fname);
    });
  }
  ...
  openWindow(e) {
    if (this.__openWindow && !this.__openWindow.closed) {
      ...
      this.__openWindow = globalThis.open("", "studentwork", `left=0,top=0,width=${screen.width / 2},height=${screen.height / 2},menubar=0,location=0,toolbar=0,status=0`);
  ...
  renderSubmissionInWindow() {
    if (this.__openWindow && this.displayMode === 2) {
      ...
      render(html` ${this.database.assignments.length && this.database.assignments[this.activeAssignment] ? html`<grade-book-pop-up></grade-book-pop-up>` : html`${this.t.noSubmission}`}`, this.__openWindow.document.body);
    }
  }
  /**
  ...
  render() {
    return html`
      ...
      <div class="top-controls">
        ...
        ${this.ready && this.source === "filesystem" && this.sourceData ? html`
              <div class="group">
                <simple-icon-button-lite
      ...
      <div ?hidden="${this.sourceData}" class="source-selection">
        ...
        <select id="source" @change="${this.selectSource}">
          ${this.hasFilePicker ? html`<option
                value="filesystem"
        ...
        ${this.source == "filesystem" && this.prevLocalFileReference ? html`
              <simple-icon-button-lite
      ...
      ${this.source === "filesystem" && this.loading ? html`<p class="source-selection">${this.t.loadingFilePleaseWait}</p>` : nothing}
      <table
        id="studentgrid"
      ...
      >
        ${this.database.roster.length && this.database.assignments.length ? html`
              <colgroup>
                <col />
              </colgroup>
              ${this.database.assignments.map((a, h) => html`<colgroup>
                    <col />
                  </colgroup>`)}
              <thead class="thead">
                <tr class="tr thead-tr" part="tr">
                  ...
                  ${this.database.assignments.map((a, h) => html`<th
              ...
              <tbody class="tbody">
                ${this.database.roster.map((s, i) => html` <tr class="tr tbody-tr">
                      <th class="th th-or-td" data-assignment="-1">
                        <div class="user-info">
                          <div class="user-left">
                            ${this.settings.photo ? html`${s.photo ? html`<img
                                      src="${s.photo}"
                                      loading="lazy"
                                      class="user-photo"
                                    />` : html`<simple-icon-lite
                                      icon="social:person"
                                      class="user-photo"
                                    ></simple-icon-lite>`}` : nothing}
                          </div>
                          <div class="user-right">
                            ${this.settings.fname ? html`<div>${s.student}</div>` : nothing}
                            ${this.settings.surname ? html`<div>${s.student}</div>` : nothing}
                            ${this.settings.email ? html`<div>
                      ...
                      ${this.database.assignments.map((a, h) => html` <td
                            class="td th-or-td"
                            numeric
                            data-student="${i}"
                            data-assignment="${h}"
                            ?data-active="${this.activeStudent === i && this.activeAssignment === h}"
                          >
                            <button
                              ...
                              data-assignment="${h}"
                            >
                              ${this.database.grades[i] && this.database.grades[i][a.shortName] ? html`${this.database.grades[i][a.shortName]}` : `-`}
                            </button>
                          </td>`)}
                    </tr>`)}
              </tbody>
          ...
          ${this.activeRubric[0] ? html`
                ...
                >
                  <div slot="col-1" class="tag-group">
                    ...
                    ${this.database.tags.categories.length > 0 ? html`
                          <a11y-collapse-group heading-button>
                            ${this.database.tags.categories.map((category, i) => html`
                                <a11y-collapse>
                                  <div slot="heading">
                                    <simple-colors
                                  ...
                                  <div slot="content">
                                    ${this.database.tags.data.filter(item => {
      return item.category.includes(category);
    }).map(term => html`<simple-tag
                                            draggable="true"
                                            tabindex="0"
                  ...
                  <div slot="col-2">
                    ...
                    ${this.activeRubric.map((rubric, index) => html`
                        ${this.renderRubricInfoBtn(index, rubric.criteria, rubric.description)}
                        ...
                        >
                          <table>
                            <tbody>
                              <tr>
                                ${rubric.qualitative.map(cat => html` <td>${cat}</td> `)}
                              </tr>
                              <tr>
                                ${rubric.qualitative.map(cat => html`
                                    <td>
                                      <simple-fields-tag-list
                  ...
                  ${this.displayMode != 2 ? html`
                        <div slot="col-3">
                          ${this.database.assignments.length && this.database.assignments[this.activeAssignment] ? html`${this.activeSubmission ? this.renderSubmission(this.activeSubmission) : html`${this.t.noSubmission}`}` : nothing}
                        </div>
                      ` : nothing}
                </grid-plate>
              ` : nothing}
        ...
        >
          ...
          <div id="studentreport">
            ${!this.loading && this.database.assignments && this.database.assignments[this.activeAssignment] ? html`
                  <letter-grade
                  ...
                  <div class="student-report-wrap">
                    <a11y-collapse-group
                      ...
                      style="width:80%;"
                    >
                      ${this.database.rubrics.filter(item => {
      return item.shortName == this.database.assignments[this.activeAssignment].rubric;
    }).map(rubric => html`
                              ...
                              <div slot="content">
                                <div class="student-feedback-score-heading">
                                  <div>
                                    ...
                                    ${Math.round(rubric.percentage / 100 * this.database.assignments[this.activeAssignment].points)}
                                  </div>
                                ...
                                <ul>
                                  ${rubric.qualitative.map(cat => html`
                                      <h4>${cat}</h4>
                                      <ul>
                                        ${this.activeGrading[rubric.criteria] ? html`${this.activeGrading[rubric.criteria][cat].map(tag => html` <li>
                                                  <span>${tag.term}</span
                                                  >${tag.description ? html` - ${tag.description}` : ``}
                                                  ${tag.associatedMaterial ? html`
                                                        <ul>
                                                          ${tag.associatedMaterial.map(material => html`
                                                              <li>
                                                                <a
                                                                  href="${material}"
                                                                  target="_blank"
  ...
  loadFromFilesystem(existing = false) {
    ...
    if (existing) {
      this.loading = true;
      setTimeout(() => {
        XLSXFileSystemBrokerSingleton.processFile(this.prevLocalFileReference, "json");
  ...
  async saveToFilesystem(e) {
    ...
    const output = XLSXFileSystemBrokerSingleton.workbookFromJSON(this.database);
  ...
  updateTotalScore() {
    let score = 0;
    let tables = this.shadowRoot.querySelectorAll("#assessment simple-fields-field[type='number']:not(#totalpts)");
  ...
  getCriteriaScore(criteria) {
    let tables = this.shadowRoot.querySelectorAll("#assessment editable-table-display");
    // add the scores up based on values of the pieces
    for (var i in Array.from(tables)) {
      if (tables[i].shadowRoot.querySelector(`[data-rubric-score][data-criteria="${criteria}"]`)) {
        return tables[i].shadowRoot.querySelector(`[data-rubric-score][data-criteria="${criteria}"]`).value;
      }
    }
    return 0;
  }
  ...
  getCriteriaFeedback(criteria) {
    if (this.shadowRoot.querySelector(`#assessment [data-rubric-written][data-criteria="${criteria}"]`)) {
      return this.shadowRoot.querySelector(`#assessment [data-rubric-written][data-criteria="${criteria}"]`).value;
    }
    return "";
  }
  /**
  ...
  _handleDragDrop(e) {
    globalThis.dispatchEvent(new CustomEvent("simple-tag-drop", {
      detail: {
        value: "drop"
      }
    }));
  ...
  setDragTransfer(e) {
    globalThis.dispatchEvent(new CustomEvent("simple-tag-dragstart", {
      detail: {
        value: e.target
      }
    }));
