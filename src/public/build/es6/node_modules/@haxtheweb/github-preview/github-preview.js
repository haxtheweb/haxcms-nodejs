...
class GithubPreview extends IntersectionObserverMixin(LitElement) {
  ...
  render() {
    return this.__assetAvailable && this.elementVisible ? this.extended ? html`
            <div class="container">
   ...
   */
  fetchGithubData(repo, org, headers, branch, rawUrl, apiUrl, readMe, extended) {
    if (extended) {
      fetch(`${rawUrl}/${org}/${repo}/${branch}/${readMe}`, headers).then(response => {
        if (response.ok) {
    ...
    fetch(`${apiUrl}/repos/${org}/${repo}`).then(response => {
      if (response.ok) {
  ...
  updated(changedProperties) {
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (["repo", "org", "headers", "branch", "rawUrl", "apiUrl", "readMe", "extended"].includes(propName) && this[propName]) {
        clearTimeout(this.__debounce);
        this.__debounce = setTimeout(() => {
          this.fetchGithubData(this.repo, this.org, this.headers, this.branch, this.rawUrl, this.apiUrl, this.readMe, this.extended);
        }, 0);
      }
      ...
      if (this.elementVisible && this.extended && propName === "elementVisible") {
        this.wcmarkdown = this.shadowRoot.querySelector("div > div.readme-container > wc-markdown");
      }
    });
  }
}
