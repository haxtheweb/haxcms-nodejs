...
export class MarkTheWords extends QuestionElement {
  ...
  rebuildWordList(statement) {
    ...
    for (var i in wordList) {
      let answerMatch = this.displayedAnswers.find(answer => answer.label && wordList[i].toLowerCase() === answer.label.toLowerCase());
      this.wordList.push({
        text: wordList[i],
        selected: false,
        correct: null,
  ...
  renderFeedback() {
    return html` ${!this.edit ? html`
          ${this.showAnswer ? html` <p class="feedback">
          ...
          ${this.showAnswer && this.numberCorrect === this.displayedAnswers.filter(answer => answer.correct).length ? html` <p class="feedback">${this.correctText}</p>
                ${this.querySelector && this.querySelector('[slot="feedbackCorrect"]') ? html`<slot name="feedbackCorrect"></slot>` : ``}` : ``}
          ${this.querySelector && this.querySelector('[slot="hint"]') && this.showAnswer && this.numberCorrect !== this.displayedAnswers.filter(answer => answer.correct).length ? html`
                <h4>Need a hint?</h4>
                <div>
                  <slot name="hint"></slot>
                </div>
              ` : ``}
          ${this.querySelector && this.querySelector('[slot="evidence"]') && this.showAnswer && this.numberCorrect === this.displayedAnswers.filter(answer => answer.correct).length ? html`
                <h4>Evidence</h4>
                <div>
