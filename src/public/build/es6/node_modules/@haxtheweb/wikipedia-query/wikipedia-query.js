...
class WikipediaQuery extends IntersectionObserverMixin(LitElement) {
  ...
  constructor() {
    ...
    const language = globalThis.document.body.getAttribute("xml:lang") || globalThis.document.body.getAttribute("lang") || globalThis.document.documentElement.getAttribute("xml:lang") || globalThis.document.documentElement.getAttribute("lang") || globalThis.navigator.language || FALLBACK_LANG;
  ...
  render() {
    return html` ${this.elementVisible ? html` <h3 .hidden="${this.hideTitle}" part="heading-3">
            ${this._title}
          </h3>
          <div id="result"></div>
          <citation-element
  ...
  updateArticle(search, headers, language) {
    fetch(`https://${language}.wikipedia.org/w/api.php?origin=*&action=query&titles=${search}&prop=extracts&format=json`, headers).then(response => {
  ...
  updated(changedProperties) {
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (["elementVisible", "search", "headers", "language"].includes(propName) && this.search && this.headers && this.elementVisible && this.language) {
  ...
  handleResponse(response) {
    ...
    if (typeof response !== typeof undefined && response.query) {
      for (var key in response.query.pages) {
        ...
        if (response.query.pages[key].extract && this.shadowRoot && this.shadowRoot.querySelector("#result")) {
  ...
  haxgizmoRegistration(store) {
    ...
    if (store.appList.filter((el, i) => {
      ...
      globalThis.dispatchEvent(new CustomEvent("hax-register-app", {
        bubbles: false,
        composed: false,
        cancelable: false,
