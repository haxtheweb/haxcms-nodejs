...
globalThis.AbsolutePositionStateManager.requestAvailability = () => {
  if (!globalThis.AbsolutePositionStateManager.instance && globalThis.document) {
    globalThis.AbsolutePositionStateManager.instance = globalThis.document.createElement("absolute-position-state-manager");
...
class AbsolutePositionStateManager extends LitElement {
  ...
  _handleScroll() {
    ...
    this.__timeout2 = setTimeout(globalThis.AbsolutePositionStateManager.instance.updateStickyElements(), 1000);
  }
  ...
  _handleResize() {
    ...
    this.__timeout = setTimeout(globalThis.AbsolutePositionStateManager.instance.updateElements(), 250);
  }
  ...
  removeEventListeners() {
    ...
    if (this.__watchSticky && this.scrollTarget) this.scrollTarget.removeEventListener("scroll", this._handleScroll);
  }
  ...
  loadSticky() {
    //only have event listeners when there are elements using manager
    if (!this.__watchSticky && this.elements.filter(el => el.sticky).length > 0 && this.scrollTarget) {
      this.__watchSticky = true;
      this.scrollTarget.addEventListener("scroll", this._handleScroll, {
        passive: true
      });
    } else if (this.__watchSticky && this.elements.filter(el => el.sticky).length < 1 && this.scrollTarget) {
  ...
  positionElement(el) {
    ...
    let w = globalThis.document.body.getBoundingClientRect(),
      ...
      setAlign = (v = vertical(el.position)) => {
        ...
        let min = v ? pxToNum(el.style.left) - e.left : pxToNum(el.style.top) - e.top,
          startAt = v ? "left" : "top",
      ...
      getCoord = (pos = el.position) => {
        let coord,
          ...
          eh = !el.allowOverlap && globalThis.getComputedStyle(el, null).overflowY == "visible" ? Math.max(e.height, el.scrollHeight) : e.height,
          ew = !el.allowOverlap && globalThis.getComputedStyle(el, null).overflowX == "visible" ? Math.max(e.width, el.scrollWidth) : e.width;
    ...
    if (el.sticky) {
      let scrollTop = globalThis.pageYOffset || (globalThis.document.documentElement || globalThis.document.body.parentNode || globalThis.document.body).scrollTop,
