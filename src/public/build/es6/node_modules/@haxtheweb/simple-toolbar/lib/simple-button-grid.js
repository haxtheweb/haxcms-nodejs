...
const SimpleButtonGridBehaviors = function (SuperClass) {
  return class extends SimpleToolbarBehaviors(SuperClass) {
    ...
    get gridStyles() {
      let styles = [];
      if (!!this.columns) styles.push(`--simple-button-grid-cols:${100 / this.columns}%`);
      return styles.join(";");
    }
    ...
    resizeToolbar() {
      ...
      let more = !this.shadowRoot ? undefined : this.shadowRoot.querySelector("#morebutton"),
      ...
      items.forEach(item => {
        ...
        if (rows.length == 0) {
          ...
          if (!!this.rows && rows.length > this.rows || !this.rows && newrow && this._bottom(item) > max) {
