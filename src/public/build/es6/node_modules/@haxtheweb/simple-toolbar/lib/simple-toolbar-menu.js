...
import { A11yMenuButtonBehaviors } from "../../a11y-menu-button/a11y-menu-button.js";
...
const SimpleToolbarMenuBehaviors = function (SuperClass) {
  return class extends A11yMenuButtonBehaviors(SimpleToolbarButtonBehaviors(SuperClass)) {
    static get simpleButtonCoreStyles() {
      return [...super.simpleButtonCoreStyles, css`
          :host {
    ...
    static get simpleButtonLayoutStyles() {
      return [...super.simpleButtonLayoutStyles, css`
          #menu {
    ...
    static get simpleButtonThemeStyles() {
      return [...super.simpleButtonThemeStyles, css`
    ...
    static get styles() {
      return [...super.menuButtonCoreStyles, ...this.simpleButtonCoreStyles, ...this.iconStyles, ...this.labelStyles, ...this.tooltipStyles, ...this.simpleButtonLayoutStyles, ...this.simpleButtonThemeStyles];
    }
     ...
     */
    get tooltipTemplate() {
      return !this.tooltipVisible ? "" : html`<simple-tooltip
            id="tooltip"
            for="menubutton"
     ...
     */
    _handleFocus(event) {
      if (SimpleToolbarButtonBehaviors._handleFocus) SimpleToolbarButtonBehaviors._handleFocus(event);
      if (A11yMenuButtonBehaviors._handleFocus) A11yMenuButtonBehaviors._handleFocus(event);
    }
     ...
     */
    _handleBlur(event) {
      if (SimpleToolbarButtonBehaviors._handleBlur) SimpleToolbarButtonBehaviors._handleBlur(event);
      if (A11yMenuButtonBehaviors._handleBlur) A11yMenuButtonBehaviors._handleBlur(event);
    ...
    addItem(item) {
      if (this.getItemIndex(item) < 0) {
        ...
        Object.keys(listeners).forEach(evt => (item.focusableElement || item).addEventListener(evt, listeners[evt].bind(this)));
      }
    }
    /**
    ...
    removeItem(item) {
      if (this.getItemIndex(item) < 0) {
        let listeners = this.itemListeners;
        Object.keys(listeners).forEach(evt => (item.focusableElement || item).removeEventListener(evt, listeners[evt].bind(this)));
        if (this.__menuItems) this.__menuItems = [...this.menuItems.filter(i => item !== i)];
      }
    }
  };
};
