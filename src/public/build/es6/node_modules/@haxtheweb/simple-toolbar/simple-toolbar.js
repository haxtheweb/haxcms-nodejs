  ...
  return class extends SuperClass {
    ...
    updated(changedProperties) {
      ...
      changedProperties.forEach((oldValue, propName) => {
        ...
        if (propName === "hidden") this.setAttribute("aria-hidden", this.hidden ? "true" : "false");
      });
    ...
    deregisterButton(button) {
      ...
      (button.shortcutKeys || "").split(" ").forEach(key => delete this.shortcutKeys[key]);
      button.removeEventListener("blur", e => this._handleFocusChange(button));
      button.removeEventListener("focus", e => this._handleFocusChange(button));
    }
    /**
    ...
    registerButton(button) {
      ...
      (button.shortcutKeys || "").split(" ").forEach(key => this.shortcutKeys[key] = button);
    ...
    _handleToggleToolbar(e) {
      this.collapsed = e.detail && typeof e.detail !== typeof undefined ? e.detail : !this.collapsed;
    }
    /**
    ...
    _renderButtonGroup(config) {
      let type = !!config.type && config.type === "simpletoolbar-button-group" ? config.type : "div";
    ...
    _shortcutKeysMatch(keyEvt) {
      ...
      Object.keys(this.shortcutKeys || {}).forEach(shortcut => {
        let keys = (shortcut || "").toLowerCase().split("+"),
          ...
          filter = keys.filter(key => key.length == 1).map(key => !!uppercase ? key.toUpperCase() : key),
          key = filter[0],
          match = altKey === keyEvt.altKey && (ctrlKey === keyEvt.ctrlKey || ctrlKey === globalThis.navigator.platform === "MacIntel" && e.metaKey) && metaKey === keyEvt.metaKey && shiftKey === keyEvt.shiftKey && (keyEvt.key ? keyEvt.key === key : !key);
