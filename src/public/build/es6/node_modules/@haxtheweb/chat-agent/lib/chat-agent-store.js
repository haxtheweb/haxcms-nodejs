...
import { autorun, configure, makeObservable, observable, toJS } from "../../../mobx/dist/mobx.esm.js";
configure({
...
class ChatAgentStore {
  constructor() {
    ...
    this.darkMode = null;
    store.darkMode !== undefined ? this.darkMode = store.darkMode : this.darkMode = false;
    this.dataCollectionEnabled = true;
    this.developerModeEnabled = false; // ! this will enable developer mode for the entire chat system
    store.editMode !== undefined ? this.editMode = store.editMode : this.editMode = false;
    this.engine = "alfred";
   ...
   */
  startAI() {
    this.handleMessage("merlin", `Hello! My name is Merlin. I am currently in beta, and may not yet be feature complete, so you may encounter some bugs. By default I can answer questions about ${this.context}. How can I assist you today?`);
    if (this.month === 2 && this.day === 12 || this.month === 6 && this.day === 6 || this.month === 7 && this.day === 27 || this.month === 8 && this.day === 15 || this.month === 9 && this.day === 19 || this.month === 10 && this.day === 1 || this.month === 10 && this.day === 5 || this.month === 12 && this.day === 5 || this.month === 12 && this.day === 18) {
      this.currentSuggestions = [{
    ...
    try {
      document.querySelector("chat-agent").shadowRoot.querySelector("chat-interface").shadowRoot.querySelector("chat-message").shadowRoot.querySelectorAll("chat-suggestion").forEach(suggestion => {
  ...
  handleInteraction(prompt) {
    ...
    switch (prompt) {
      ...
      case "Why is my character wearing a hat?":
        ...
        this.handleMessage("merlin", "Your character is wearing a hat because today is either a special (hat related) holiday, or another special occassion!");
        break;
