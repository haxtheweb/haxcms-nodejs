...
export const PrintBranchMixin = function (SuperClass) {
  return class extends HAXCMSI18NMixin(SuperClass) {
    ...
    PrintBranchButton(position = "auto") {
      return html`
        ${MicroFrontendRegistry.has("@haxcms/siteToHtml") ? html` <div
    ...
    async printBranchOfSite(e) {
      ...
      if (response.status == 200 && response.data) {
        ...
        link.href = globalThis.URL.createObjectURL(b64toBlob(btoa(unescape(encodeURIComponent(response.data))), "text/html"));
        ...
        globalThis.open(globalThis.URL.createObjectURL(b64toBlob(btoa(unescape(encodeURIComponent(response.data))), "text/html")), "", "left=0,top=0,width=800,height=800,toolbar=0,scrollbars=0,status=0,noopener=1,noreferrer=1");
      } else {
        // fallback in case the service fails
        globalThis.open(globalThis.location.href + "?format=print-page", "", "left=0,top=0,width=800,height=800,toolbar=0,scrollbars=0,status=0,noopener=1,noreferrer=1");
      }
      this.__printBranchLoading = false;
    }
  };
