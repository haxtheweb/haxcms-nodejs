 ...
 */
import { LitElement, html } from "../../../../lit/index.js";
...
import { HAXStore } from "../../../hax-body/lib/hax-store.js";
...
class HAXCMSSiteEditor extends LitElement {
  ...
  __deleteNodeResponseChanged(e) {
    ...
    if (e.detail.value && e.detail.value.data && e.detail.value.data.title) {
      store.toast(`Page deleted ${e.detail.value.data.title}, selecting another page`, 4000);
  ...
  _handleUserDataResponse(e) {
    if (e.detail.response && e.detail.response.data) {
      ...
      this.dispatchEvent(new CustomEvent("haxcms-user-data-updated", {
        composed: true,
        bubbles: true,
        cancelable: false,
  ...
  firstUpdated(changedProperties) {
    ...
    globalThis.dispatchEvent(new CustomEvent("haxcms-site-editor-loaded", {
      bubbles: true,
      composed: true,
      cancelable: false,
      detail: true
  ...
  connectedCallback() {
    ...
    globalThis.addEventListener("hax-store-ready", this._storeReadyToGo.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("json-outline-schema-active-item-changed", this._newActiveItem.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("json-outline-schema-active-body-changed", this._bodyChanged.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-save-outline", this.saveOutline.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-save-node", this.saveNode.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-save-site-data", this.saveManifest.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-load-site-dashboard", this.loadSiteDashboard.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-load-user-data", this.loadUserData.bind(this), {
      signal: this.windowControllers.signal
    });
  ...
  async createNode(e) {
    if (e.detail.values) {
      ...
      if (reqBody.docximport) {
        await import("../../../file-system-broker/lib/docx-file-system-broker.js").then(async e => {
          ...
          const response = await MicroFrontendRegistry.call("@haxcms/docxToSite", formData);
  ...
  _publishingChanged(newValue, oldValue) {
    if (newValue) {
      store.toast(`Publishing...`, 0, {
        hat: "random"
      });
    } else if (!newValue && oldValue) {
      store.toast(`Publishing...`, 2000, {
        hat: "random"
      });
    }
  }
  ...
  _manifestChanged(newValue) {
    if (this.activeItem && newValue.metadata) {
      ...
      HAXStore.connectionRewrites.appendUploadEndPoint = "siteName=" + newValue.metadata.site.name + "&nodeId=" + this.activeItem.id;
    }
  }
  /**
  ...
  _activeItemChanged(newValue, oldValue) {
    if (newValue && this.manifest) {
      // set upload manager to point to this location in a more dynamic fashion
      HAXStore.connectionRewrites.appendUploadEndPoint = "siteName=" + this.manifest.metadata.site.name + "&nodeId=" + newValue.id;
    }
  }
  /**
  ...
  _handleOutlineResponse(e) {
    ...
    store.toast(`Outline saved!`, 3000, {
      hat: "random"
    });
    store.playSound("coin");
    setTimeout(() => {
  ...
  saveManifest(e) {
    ...
    if (values.cssVariable) {
      values.hexCode = globalThis.SimpleColorsSharedStyles.colors[values.cssVariable.replace("--simple-colors-default-theme-", "").replace("-7", "")][6];
