...
class SiteGitCorner extends HAXCMSI18NMixin(HAXCMSThemeParts(LitElement)) {
  ...
  constructor() {
    ...
    this.__disposer = [];
    autorun(reaction => {
      if (varGet(store.manifest, "metadata.site.git.publicRepoUrl", "") != "" && !globalThis.customElements.get("git-corner")) {
    ...
    autorun(reaction => {
      if (store.activeItem) {
        let filePath = varGet(store.manifest, "metadata.site.git.publicRepoUrl", "") + store.activeItem.location;
