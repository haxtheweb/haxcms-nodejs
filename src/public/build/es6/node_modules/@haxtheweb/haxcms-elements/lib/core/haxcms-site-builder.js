...
import { encapScript, findTagsInHTML, wipeSlot, varExists, localStorageSet, validURL } from "../../../utils/utils.js";
...
import "../../../simple-progress/simple-progress.js";
...
import "../../../replace-tag/replace-tag.js";
...
class HAXCMSSiteBuilder extends I18NMixin(LitElement) {
  ...
  nodeNormalizeIDs(node) {
    if (node.tagName && node.getAttribute("id") == null && ["H1", "H2", "H3", "H4", "H5", "H6"].includes(node.tagName)) {
      if (node.getAttribute("resource")) {
        ...
        let id = node.tagName.toLowerCase() + "-" + this.hashCode(node.innerText);
  ...
  lastErrorChanged(e) {
    if (e) {
      ...
      if (e.detail && e.detail.value) {
        ...
        if (window && globalThis.location && globalThis.appSettings && globalThis.appSettings.reloadOnError) {
          globalThis.location.reload();
        }
        store.toast(e.detail.value.status + " " + e.detail.value.statusText, 5000, {
          fire: true
        });
      } else {
  ...
  constructor() {
    ...
    this.registerLocalization({
      context: this,
      namespace: "haxcms",
      localesPath: new URL("../../locales/haxcms.es.json", import.meta.url).href.replace("/haxcms.es.json", "/"),
      locales: ["es"]
    });
    ...
    globalThis.addEventListener("haxcms-trigger-update", this._triggerUpdatedData.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-trigger-update-node", this._triggerUpdatedNode.bind(this), {
      signal: this.windowControllers.signal
    });
    ...
    autorun(() => {
      ...
      if (memory.editMode === false && memory.interactionDelay >= 3600 && usAction.type === null && store.cmsSiteEditor.haxCmsSiteEditorUIElement && store.cmsSiteEditor.haxCmsSiteEditorUIElement.shadowRoot) {
      ...
      if (UserScaffoldInstance.active && UserScaffoldInstance.memory.isLoggedIn && UserScaffoldInstance.memory.recentTarget === SuperDaemonInstance && SuperDaemonInstance.programName === null && UserScaffoldInstance.memory.interactionDelay > 600 && ["paste", "key"].includes(usAction.type)) {
        if (validURL(SuperDaemonInstance.value)) {
          SuperDaemonInstance.waveWand([SuperDaemonInstance.value, "/", {}, "hax-agent", "Agent"], null, "coin2");
        }
      } else if (UserScaffoldInstance.active && UserScaffoldInstance.memory.isLoggedIn && SuperDaemonInstance.programName === null && ["paste"].includes(usAction.type) && UserScaffoldInstance.data.architype == "url") {
  ...
  firstUpdated(changedProperties) {
    ...
    this.__ready = true;
    store.appReady = true;
    globalThis.dispatchEvent(new CustomEvent("haxcms-ready", {
      bubbles: true,
      composed: true,
      cancelable: false,
      detail: this
    ...
    import("./haxcms-editor-builder.js").then(response => {
      ...
      if (HAXcmsStore.getApplicationContext() !== "published") {
        this._timeStamp = Math.floor(Date.now() / 1000);
      } else {
        this._timeStamp = "";
      }
  ...
  storeReady(e) {
    ...
    if (store.cmsSiteEditor && store.cmsSiteEditor.instance && globalThis.HaxStore.requestAvailability().activeHaxBody && store.activeItemContent) {
      globalThis.HaxStore.requestAvailability().activeHaxBody.importContent(store.activeItemContent);
    }
  }
  /**
      ...
      if (html !== null && store.activeItem && store.activeItem.metadata) {
        ...
        setTimeout(() => {
          if (store.themeElement.childNodes.length === 0) {
            let frag = document.createRange().createContextualFragment(this.replaceTagReplacement(html));
            store.themeElement.appendChild(frag);
          ...
          if (!globalThis.WCAutoload && varExists(this.manifest, "metadata.node.dynamicElementLoader")) {
            let tagsFound = findTagsInHTML(html);
            const basePath = new URL("./locales/haxcms.es.json", import.meta.url).href + "/../";
            for (var i in tagsFound) {
              const tagName = tagsFound[i];
              if (this.manifest.metadata.node.dynamicElementLoader[tagName] && !globalThis.customElements.get(tagName)) {
                // prettier-ignore
                import(`${basePath}../../../../${this.manifest.metadata.node.dynamicElementLoader[tagName]}`).then(response => {
    ...
    if (newValue) {
      ...
      if (typeof this.__imported[theme.element] !== typeof undefined) {
        ...
        if (globalThis.WCAutoload) {
          ...
          setTimeout(() => {
            globalThis.WCAutoload.process();
            globalThis.dispatchEvent(new CustomEvent("haxcms-theme-ready", {
              bubbles: true,
              composed: true,
              cancelable: false,
              detail: this
...
globalThis.HAXme = function (context = null) {
  ...
  if (globalThis.document.body) {
    globalThis.document.body.querySelector("haxcms-editor-builder").__appliedContext = false;
