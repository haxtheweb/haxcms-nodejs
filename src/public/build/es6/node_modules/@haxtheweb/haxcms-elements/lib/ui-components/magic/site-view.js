...
import "../../../../simple-fields/lib/simple-tags.js";
...
import "../../../../play-list/play-list.js";
import "../site/site-remote-content.js";
...
export const mediaKeys = ["audio", "selfChecks", "objectives", "authorNotes", "images", "h5p", "headings", "dataTables", "specialTags", "links", "placeholders", "siteremotecontent", "video"];
...
export class SiteView extends SimpleColors {
  ...
  rebuildSearchResults() {
    ...
    this._searchDebounce = setTimeout(async () => {
      if (this.shadowRoot && !this.loading) {
        ...
        this.loading = true;
        const response = await MicroFrontendRegistry.call("@haxcms/views", params);
        if (response.data) {
  ...
  render() {
    return html` ${this.loading ? html`<div class="loading">
          ${this.loading ? html`<simple-icon-lite icon="hax:loading"></simple-icon-lite>` : ``}
        </div> ` : html` ${this.results.length === 0 && !this.loading ? html`<h4>No results found</h4>` : nothing}
  ...
  listTemplate() {
    return html` <ul class="list">
      ${this.results.map(item => html` <li class="list-item">
            ${this.params.displayOf === "blocks" ? mediaKeys.map(key => item.media && item.media[key] && typeof item.media[key] == "string" && this.params.blockFilter === key ? unsafeHTML(item.media[key]) : nothing) : html`<div class="play-list-item">
                  ${this.params.displayOf === "title" ? html`<div class="list-link">
                          <a href="${item.slug}">${item.title}</a>
                        </div>
                        <div class="list-breadcrumb">
                          ${this.calculateBreadcrumb(item).map(item => html` <span>${item.title}</span> `)}
                        </div>
                        ${item.metadata.tags && item.metadata.tags != "" ? item.metadata.tags.split(",").map(tag => html`<a href="x/views?tags=${tag.trim()}">
                                    <simple-tag
                                      auto-accent-color
                                      value="${tag.trim()}"
                                      accent-color="${item.accentColor}"
                                    ></simple-tag
                                  ></a>`) : nothing}` : this.params.displayOf === "full" ? unsafeHTML(`<h3>${item.title}</h3>` + item.contents) : html`<site-remote-content
                          player
                          hide-reference
                          uuid="${item.id}"
                          show-title
  ...
  tableTemplate(dark, accentColor) {
    ...
    >
      <table>
        ...
        ${this.results.map(item => html` <tr>
              <td>
                ${item.metadata.pageType ? html`<simple-icon
                      ?dark="${dark}"
              ...
              <td>
                ${item.metadata.tags && item.metadata.tags != "" ? item.metadata.tags.split(",").map(tag => html`<a
                            part="tag-link"
  ...
  renderPlayListTemplate() {
    ...
    render(html`${this.results.map(item => html` ${this.params.displayOf === "blocks" ? mediaKeys.map(key => item.media && item.media[key] && typeof item.media[key] == "string" && this.params.blockFilter === key ? unsafeHTML(item.media[key]) : nothing) : html`<div class="play-list-item">
                ${this.params.displayOf === "title" ? html`<div class="list-link">
                        <a href="${item.slug}">${item.title}</a>
                      </div>
                      <div class="list-breadcrumb">
                        ${this.calculateBreadcrumb(item).map(item => html` <span>${item.title}</span> `)}
                      </div>` : this.params.displayOf === "full" ? unsafeHTML(`<h3>${item.title}</h3>` + item.contents) : html`<site-remote-content
                        player
                        hide-reference
                        uuid="${item.id}"
                        show-title
  ...
  cardTemplate() {
    return html`${this.results.map(item => html` <accent-card accent-color="red" horizontal accent-heading>
