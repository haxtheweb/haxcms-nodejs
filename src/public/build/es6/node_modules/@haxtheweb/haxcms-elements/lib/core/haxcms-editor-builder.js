...
class HAXCMSEditorBuilder extends HTMLElement {
  ...
  constructor() {
    ...
    this.applyContext();
    globalThis.addEventListener("haxcms-site-editor-loaded", this.editorLoaded.bind(this), {
  ...
  editorLoaded(e) {
    if (!store.cmsSiteEditor.haxCmsSiteEditorUIElement) {
      // prettier-ignore
      import("./haxcms-site-editor-ui.js").then(() => {
        store.cmsSiteEditor.haxCmsSiteEditorUIElement = globalThis.document.createElement("haxcms-site-editor-ui");
        for (var key in store.setupSlots) {
          switch (key) {
            ...
            case "haxcms-site-editor-ui-topbar-character-button":
              for (var key2 in store.setupSlots[key]) store.cmsSiteEditor.haxCmsSiteEditorUIElement.appendChild(store.setupSlots[key][key2]);
              break;
          }
        }
        if (this.parentNode) {
  ...
  async applyContext(context = null) {
    if (!this.__appliedContext) {
      ...
      if (!["published", "11ty"].includes(context) && this.__hasConnectionSettings) {
        const basePath = new URL("./haxcms-editor-builder.js", import.meta.url).href + "/../";
        ...
        import(`${basePath}backends/${store.cmsSiteEditorBackend.tag}.js`).then(e => {
          if (!store.cmsSiteEditorBackend.instance) {
            store.cmsSiteEditorBackend.instance = globalThis.document.createElement(store.cmsSiteEditorBackend.tag);
