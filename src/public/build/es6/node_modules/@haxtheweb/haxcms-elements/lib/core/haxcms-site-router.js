...
import { autorun } from "../../../../mobx/dist/mobx.esm.js";
...
class HAXCMSSiteRouter extends HTMLElement {
  ...
  constructor() {
    ...
    globalThis.addEventListener("vaadin-router-location-changed", this._routerLocationChanged.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("haxcms-site-router-add", this.addRoutesEvent.bind(this), {
      signal: this.windowControllers.signal
    });
  }
  /**
  ...
  _routerLocationChanged(e) {
    ...
    else if (e.detail.location.route.name === "home" && e.detail.location.search.startsWith("?p=/") && this.lookupRoute(e.detail.location.search.replace("?p=/", "").split("&")[0])) {
        let item = this.lookupRoute(e.detail.location.search.replace("?p=/", "").split("&")[0])[0];
        store.activeId = item.id;
        globalThis.history.replaceState({}, null, e.detail.location.search.replace("?p=/", "").split("&")[0]);
