import { observable, makeObservable, computed, autorun, toJS, configure } from "../../../../mobx/dist/mobx.esm.js";
import { varExists, varGet, localStorageGet } from "../../../utils/utils.js";
import { JsonOutlineSchema } from "../../../json-outline-schema/json-outline-schema.js";
import { DeviceDetails } from "../../../replace-tag/lib/PerformanceDetect.js";
import { iconFromPageType } from "../../../course-design/lib/learning-component.js";
...
class Store {
  constructor() {
    ...
    this.darkMode = !localStorageGet("app-hax-darkMode") ? false : localStorageGet("app-hax-darkMode");
    this.setupSlots = {
    ...
    fetch(new URL("../../package.json", import.meta.url)).then(response => response.json()).then(obj => this.version = obj.version);
    this.appReady = false;
    this.editMode = false;
    this.manifest = null;
    ...
    makeObservable(this, {
      ...
      internalRoutes: observable,
      // internal routes to haxcms
      editMode: observable,
      ...
      // theme element
      version: observable,
      // version of haxcms FRONTEND as per package.json
      ...
      regionData: computed,
      // get the active region data from manifest + activeId
      entityData: computed,
      // get entity data from manifest
      homeLink: computed,
      activeId: observable,
      ...
      try {
        switch (sound) {
          ...
          case "error":
            ...
            this.audio = new Audio(new URL(`../../../app-hax/lib/assets/sounds/${sound}.mp3`, import.meta.url).href);
            this.audio.volume = 0.3;
            this.audio.play();
            break;
          default:
            this.audio = new Audio(new URL(`../../../app-hax/lib/assets/sounds/hit.mp3`, import.meta.url).href);
            this.audio.volume = 0.3;
            this.audio.play();
  ...
  toast(message, duration = 2000, extras = {}, classStyle = "capsule", closeText = this.t.close, eventCallback = null, slot = null) {
    if (this.appReady) {
      ...
      globalThis.dispatchEvent(new CustomEvent("haxcms-toast-show", {
        bubbles: true,
        composed: true,
        cancelable: true,
        detail: {
  ...
  async loadManifest(manifest, target = null) {
    ...
    if (globalThis.document.documentElement && manifest.metadata.site.lang) {
      ...
      globalThis.dispatchEvent(new CustomEvent("languagechange", {
    ...
    globalThis.dispatchEvent(new CustomEvent("haxcms-item-rebuild", {
      bubbles: true,
      composed: true,
      cancelable: false,
      detail: true
  ...
  cmsSiteEditorAvailability() {
    if (!this.cmsSiteEditor.instance) {
      this.cmsSiteEditor.instance = globalThis.document.createElement("haxcms-site-editor");
    }
  ...
  get routerManifest() {
    ...
    globalThis.document.body.dispatchEvent(new CustomEvent("json-outline-schema-changed", {
      bubbles: true,
      composed: true,
      cancelable: false,
      detail: manifest
    ...
    if (manifest && typeof manifest.items !== "undefined") {
       ...
       * item.
       */
      if (varGet(manifest, "metadata.site.settings.publishPagesOn", true) === true) {
        ...
        case "404":
          ...
          if (internalRouteTest && store.internalRoutes[internalRouteTest]) {
            ...
            item = {
              ...
              title: this.t[internalRouteTest] || internalRouteTest,
              // translation otherwise just the route name
  ...
  get parentTitle() {
    if (this.manifest && this.activeItem) {
      let tmpItem = this.manifest.items.find(d => this.activeItem.parent === d.id);
      // shift up 1 if we found something
      if (tmpItem) {
   ...
   * shortcut to find an item in the manifest based on id
   */
  async findItemAsObject(id, attrLookup = "id", scope = "item", useToJS = true) {
    if (this.manifest && id) {
      ...
      if (scope == "item") {
        ...
        return toJS(await this.manifest.items.find(d => tmpItem.parent === d.id));
      }
    }
    return null;
  }
  ...
  async addItem(item) {
    ...
    // build a new array in the correct order by pushing the old items around
    for (var key in correctOrder) {
      newItems.push(schema.items.find(element => {
        return element.id === correctOrder[key].id;
      }));
    }
    this.manifest.items.replace(newItems);
    globalThis.dispatchEvent(new CustomEvent("json-outline-schema-changed", {
      bubbles: true,
      composed: true,
      cancelable: false,
  ...
  spiderChildren(item, data, start, end, parent, parentFound, noDynamicLevel) {
    ...
    if (item.id === parent || parentFound) {
      ...
      if (item.children.length > 0) {
        item.children.forEach(child => {
          // recursively call itself
          this.spiderChildren(child, data, end, parent, parentFound, noDynamicLevel);
        });
      }
    }
  }
...
class HAXCMSSiteStore extends HTMLElement {
  constructor() {
    ...
    autorun(() => {
      ...
      if (foundItem) {
        globalThis.document.body.dispatchEvent(new CustomEvent("json-outline-schema-active-item-changed", {
          bubbles: true,
          composed: true,
          cancelable: false,
          detail: foundItem
    ...
    autorun(() => {
      ...
      if (globalThis.HaxStore && globalThis.HaxStore.requestAvailability() && globalThis.HaxStore.requestAvailability().write) {
        globalThis.dispatchEvent(new CustomEvent("haxcms-edit-mode-changed", {
          bubbles: true,
          composed: true,
          cancelable: false,
          detail: editMode
        ...
        globalThis.HaxStore.requestAvailability().toastShowEventName = "haxcms-toast-show";
      }
    });
  }
  // play sound on event
  ...
  getApplicationContext() {
    ...
    if (typeof DatArchive !== typeof undefined) {
      ...
      switch (globalThis.HAXCMSContext) {
        ...
        case "userfs":
          // implies hax.cloud stylee usage pattern
