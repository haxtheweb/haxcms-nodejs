...
import { store, HAXcmsStore } from "../../core/haxcms-site-store.js";
import { HAXCMSI18NMixin } from "../../core/utils/HAXCMSI18NMixin.js";
import { DDD } from "../../../../d-d-d/d-d-d.js";
import "../../../../simple-fields/lib/simple-tag.js";
...
class SiteSearch extends HAXCMSI18NMixin(DDD) {
    ...
    return html`
      ...
      ${this.__results.map(item => html`
          ...
          >
            ...
            ${item.breadcrumb ? html`<div>
                  ${item.breadcrumb.map((crumb, i) => html`${i != 0 ? html`<simple-icon-lite
  ...
  _searchValueChanged(e) {
    ...
    if (this.search) {
      ...
      globalThis.history.replaceState({}, "", decodeURIComponent(`./x/search?${params}`));
    }
  }
  async __resultsChanged(e) {
    if (e.detail.value) {
      ...
      await results.map(async item => {
        let fullItem = await store.findItemAsObject(item.location, "slug", "item");
        var breadcrumb = [{
        ...
        while (itemBuilder && itemBuilder.parent != null) {
          itemBuilder = await store.manifest.items.find(i => i.id == itemBuilder.parent);
