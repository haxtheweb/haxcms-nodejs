...
const HAXCMSRememberRoute = function (SuperClass) {
  return class extends HAXCMSI18NMixin(SuperClass) {
      ...
      autorun(reaction => {
        if (store && store.location && store.location.pathname) {
          ...
          if (activePathName && this.__evaluateRoute) {
            localStorageSet(`HAXCMSlastRoute-${store.manifest.metadata.site.name}`, activePathName);
          }
        }
      });
    }
    firstUpdated(changedProperties) {
      ...
      setTimeout(() => {
        if (store && store.location && store.location.pathname && localStorageGet(`HAXCMSlastRoute-${store.manifest.metadata.site.name}`) && localStorageGet(`HAXCMSlastRoute-${store.manifest.metadata.site.name}`) != toJS(store.location.pathname)) {
          let btn = globalThis.document.createElement("a");
          btn.setAttribute("href", localStorageGet(`HAXCMSlastRoute-${store.manifest.metadata.site.name}`));
          ...
          if (!format) {
            store.toast(this.t.resumeMessage, 5000, {}, "capsule", null, null, btn);
          }
        }
