  ...
  siteOperation(e) {
    ...
    import("../../../simple-modal/simple-modal.js").then(() => {
      setTimeout(() => {
        ...
        div.appendChild(globalThis.document.createTextNode(`Are you sure you want to ${op.replace("Site", "")} ${site.metadata.site.name}?`));
        // gitlist opens in a new window
        if (op === "gitList") {
          // php library is basis for this button, rare instance
          if (globalThis.HAXCMSContext === "php") {
            // open link in new window
            globalThis.open(`gitlist/${site.metadata.site.name}`, "_blank", "noopener noreferrer");
          }
        } else {
  ...
  async confirmOperation() {
    ...
    await store.AppHaxAPI.makeCall(op, {
      ...
      if (activeOp === "downloadSite") {
        ...
        globalThis.open(store.AppHaxAPI.lastResponse.downloadSite.data.link, "_blank");
      } else {
    ...
    store.toast(`${site.metadata.site.name} ${op.replace("Site", "")} successful!`, 3000, {
      hat: "random"
    });
  }
