...
import { localStorageSet } from "../../../utils/utils.js";
...
export class AppHaxSteps extends SimpleColors {
  ...
  async evoImport(e) {
    if (!e.target.comingSoon) {
      ...
      import("../../../file-system-broker/file-system-broker.js").then(async e => {
        ...
        this.setProcessingVisual();
        const response = await MicroFrontendRegistry.call("@haxcms/evolutionToSite", formData, null, null, "?jwt=" + toJS(store.AppHaxAPI.jwt));
        store.toast(`Processed!`, 300);
        // must be a valid response and have at least SOME html to bother attempting
        if (response.status == 200 && response.data && response.data.contents != "") {
          store.items = response.data.items;
          ...
          // send to the endpoint and wait
          // if it comes back with content, then we engineer details off of it
          this.nameTyped = response.data.filename.replace(".zip", "").replace("outline", "").replace(/\s/g, "").replace(/-/g, "").toLowerCase();
          setTimeout(() => {
            this.shadowRoot.querySelector("#sitename").value = this.nameTyped;
            this.shadowRoot.querySelector("#sitename").select();
          }, 800);
  ...
  async gbImport(e) {
    if (!e.target.comingSoon) {
      ...
      enableServices(["haxcms"]);
      this.setProcessingVisual();
      const response = await MicroFrontendRegistry.call("@haxcms/gitbookToSite", {
        md: gbURL
      });
      store.toast(`Processed!`, 300);
      // must be a valid response and have at least SOME html to bother attempting
      if (response.status == 200 && response.data && response.data.contents != "") {
        ...
        // send to the endpoint and wait
        // if it comes back with content, then we engineer details off of it
        this.nameTyped = response.data.filename.replace(/\s/g, "").replace(/-/g, "").toLowerCase();
        setTimeout(() => {
          this.shadowRoot.querySelector("#sitename").value = this.nameTyped;
          this.shadowRoot.querySelector("#sitename").select();
        }, 800);
  ...
  async notionImport(e) {
    if (!e.target.comingSoon) {
      ...
      enableServices(["haxcms"]);
      this.setProcessingVisual();
      const response = await MicroFrontendRegistry.call("@haxcms/notionToSite", {
        repoUrl: notionUrl
      });
      store.toast(`Processed!`, 300);
      // must be a valid response and have at least SOME html to bother attempting
  ...
  async pressbooksImport(e) {
    if (!e.target.comingSoon) {
      ...
      import("../../../file-system-broker/lib/docx-file-system-broker.js").then(async e => {
        ...
        formData.append("upload", file);
        this.setProcessingVisual();
        const response = await MicroFrontendRegistry.call("@haxcms/pressbooksToSite", formData);
        store.toast(`Processed!`, 300);
        // must be a valid response and have at least SOME html to bother attempting
        if (response.status == 200 && response.data && response.data.contents != "") {
          store.items = response.data.items;
          ...
          // send to the endpoint and wait
          // if it comes back with content, then we engineer details off of it
          this.nameTyped = response.data.filename.replace(".html", "").replace("outline", "").replace(/\s/g, "").replace(/-/g, "").toLowerCase();
          setTimeout(() => {
            this.shadowRoot.querySelector("#sitename").value = this.nameTyped;
            this.shadowRoot.querySelector("#sitename").select();
          }, 800);
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      // progress
      if (this.step === 5 && propName === "step" && this.shadowRoot && this._progressReady) {
        setTimeout(() => {
          this.shadowRoot.querySelector("app-hax-hat-progress").process();
        }, 600);
      }
    ...
    return html`
      <div id="container">
        <ul id="step-links">
          ...
          ${this.stepRoutes.map((item, index) => html`<li
