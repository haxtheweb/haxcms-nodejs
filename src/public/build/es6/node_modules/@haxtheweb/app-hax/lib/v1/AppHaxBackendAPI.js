...
import { localStorageGet } from "../../../utils/utils.js";
...
import { SimpleIconIconsetsManifest } from "../../../simple-icon/lib/simple-iconset-manifest.js";
...
export class AppHaxBackendAPI extends LitElement {
  ...
  constructor() {
    ...
    this.method = window && globalThis.appSettings && globalThis.appSettings.demo ? "GET" : "POST";
    this.basePath = "/";
    this.lastResponse = {};
    this.appSettings = {};
    autorun(() => {
  ...
  async makeCall(call, data = {}, save = false, callback = false) {
    if (this.appSettings && this.appSettings[call]) {
      ...
      const response = await fetch(`${urlRequest}`, options).then(response => {
        if (response.ok) {
        ...
        // we got a miss, logout cause something is wrong
            ...
            // over here.
            globalThis.dispatchEvent(new CustomEvent("jwt-login-refresh-token", {
              composed: true,
              bubbles: true,
              cancelable: false,
              detail: {
  ...
  firstUpdated(changedProperties) {
    ...
    store.newSitePromiseList = [...store.newSitePromiseList, async () => await this.makeCall("createSite", this._formatSitePostData(), true)];
  }
...
globalThis.AppHaxAPI.requestAvailability = () => {
  if (!globalThis.AppHaxAPI.instance) {
    globalThis.AppHaxAPI.instance = globalThis.document.createElement(AppHaxBackendAPI.tag);
