...
import { LitElement, html, css, nothing } from "../../../lit/index.js";
...
export class QuestionElement extends SchemaBehaviors(I18NMixin(DDDSuper(LitElement))) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (propName === "edit" && this.edit && this._haxstate) {
        ...
        ["feedbackIncorrect", "feedbackCorrect", "content", "hint", "evidence"].map(slotName => {
          if (this.querySelector && this.querySelector(`[slot="${slotName}"]`) === null) {
  ...
  checkAnswerCallback() {
    ...
    globalThis.dispatchEvent(new CustomEvent(toastShowEventName, {
      bubbles: true,
      composed: true,
      cancelable: true,
      detail: {
  ...
  _computeDisplayedAnswers(answers, randomize) {
    ...
    if (typeof answers !== typeof undefined && answers != null && answers.length > 0 && randomize && !this._haxstate) {
      let random = answers;
      ...
      <confetti-container id="confetti">
        <grid-plate layout="1-1">
          ...
          <div slot="col-2">
            ...
            ${this.querySelector && this.querySelector('[slot="content"]') && !this.edit ? html` <details ?open="${!this.showAnswer}" id="related">
                  <summary>
                    <simple-icon-lite
                      class="details-icon"
                      icon="lrn:content"
      ...
      <fieldset class="options">
        ${this.displayedAnswers.map((answer, index) => html`
            <simple-fields-field
              ...
              class="tag-option ${answer && this.showAnswer && answer.userGuess ? answer.correct ? "correct" : "incorrect" : ""}"
  ...
  renderFeedback() {
    return html`${!this.edit ? html`
          ${this.showAnswer && !this.isCorrect() ? html` <p class="feedback">${this.incorrectText}</p>
                ${this.querySelector && this.querySelector('[slot="feedbackIncorrect"]') ? html`<slot name="feedbackIncorrect"></slot>` : nothing}` : nothing}
          ${this.showAnswer && this.isCorrect() ? html` <p class="feedback">${this.correctText}</p>
                ${this.querySelector && this.querySelector('[slot="feedbackCorrect"]') ? html`<slot name="feedbackCorrect"></slot>` : nothing}` : nothing}
          ${this.querySelector && this.querySelector('[slot="hint"]') && this.showAnswer && !this.isCorrect() ? html`
                <h4>Need a hint?</h4>
                <div>
                  <slot name="hint"></slot>
                </div>
              ` : nothing}
          ${this.querySelector && this.querySelector('[slot="evidence"]') && this.showAnswer && this.isCorrect() ? html`
                <h4>Evidence</h4>
                <div>
  ...
  processInput(index, inputs, answers) {
    ...
    return {
      ...
      // stores the original order this was in for things that leverage this piece of data
      label: input.value,
      correct: input.getAttribute("correct") == null ? false : true,
      image: input.getAttribute("data-image") || null,
      // support for image prop in questions that want it
      alt: input.getAttribute("data-image-alt") || "",
      // support for image alt w/ prop question, "" for default to ignore
