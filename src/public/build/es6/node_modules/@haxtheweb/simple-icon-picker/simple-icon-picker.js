...
import { SimplePicker } from "../simple-picker/simple-picker.js";
...
class SimpleIconPicker extends SimplePicker {
   ...
   */
  _getStoredIcons() {
    let icons = SimpleIconsetStore && SimpleIconsetStore.iconlist ? SimpleIconsetStore.iconlist : [],
      includeSets = this.includeSets && this.includeSets.length > 0 ? typeof this.includeSets !== typeof [] ? JSON.parse(this.includeSets) : this.includeSets : false,
      excludeSets = this.excludeSets && this.excludeSets.length > 0 ? typeof this.excludeSets !== typeof [] ? JSON.parse(this.excludeSets) : this.excludeSets : false,
      exclude = this.exclude && this.exclude.length > 0 ? typeof this.exclude !== typeof [] ? JSON.parse(this.exclude) : this.exclude : false;
    if (includeSets || excludeSets || exclude) icons = icons.filter(icon => {
      let prefix = icon,
        iconname = icon,
        include = true;
      prefix = prefix.replace(/:.*/, ""), iconname.replace("icons:", "");
      if (exclude && (exclude.includes(icon) || exclude.includes(`icons:${iconname}`))) include = false;
