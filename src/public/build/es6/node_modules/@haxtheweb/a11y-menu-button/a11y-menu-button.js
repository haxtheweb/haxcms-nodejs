...
const A11yMenuButtonBehaviors = function (SuperClass) {
  return class extends SuperClass {
    ...
    constructor() {
      ...
      [...this.children].filter(n => n.slot === "menuitem").forEach(item => this.addItem(item));
      ...
      this.addEventListener("remove-a11y-menu-button-item", this._handleRemoveItem);
    }
    /**
    ...
    addItem(item) {
      ...
      Object.keys(listeners).forEach(evt => item.addEventListener(evt, listeners[evt].bind(this)));
    ...
    removeItem(item) {
      let listeners = this.itemListeners;
      if (this.menuItems) this.menuItems = [...this.menuItems.filter(i => item !== i)];
      Object.keys(listeners).forEach(evt => item.removeEventListener(evt, listeners[evt].bind(this)));
    }
    /**
    ...
    _handleClick(event) {
      ...
      if (event.pointerType === "touch") {} else {
        if (this.expanded) {
          this.close(true);
        } else {
          this.focusOn(this.firstItem);
