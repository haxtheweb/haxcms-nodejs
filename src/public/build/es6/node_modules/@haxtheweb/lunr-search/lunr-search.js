...
class LunrSearch extends LitElement {
  ...
  // render function
  render() {
    return html` ${this.demo && this.results ? html`${this.results.map(item => html`<h2>${item.title}</h2>
  ...
  constructor() {
    ...
    if (globalThis.WCGlobalBasePath) {
      ...
      this.basePath = new URL("./lunr-search.js", import.meta.url).href + "/../../../";
    }
    const location = `${this.basePath}lunr/lunr.js`;
    globalThis.addEventListener("es-bridge-lunr-loaded", this._lunrLoaded.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.ESGlobalBridge.requestAvailability().load("lunr", location);
    if (globalThis.ESGlobalBridge.requestAvailability().imports["lunr"] === true) {
      setTimeout(() => {
  ...
  updated(changedProperties) {
    changedProperties.forEach((oldValue, propName) => {
      if (["dataSource", "__auto", "method"].includes(propName) && this.dataSource && this.method) {
