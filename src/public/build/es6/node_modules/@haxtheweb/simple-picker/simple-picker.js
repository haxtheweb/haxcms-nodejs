...
const SimplePickerBehaviors = function (SuperClass) {
  return class extends SuperClass {
        ...
        >
          ...
          <div id="collapse" part="listbox">
            <div class="rows" part="listbox-rows">
              ${cache(this.expanded && this.__options ? this._renderOptions(this.__options) : nothing)}
            </div>
          </div>
        </div>`;
    }
    ...
    _isRowHidden(row) {
      return !Array.isArray(row) || row.filter(col => !!col.value || !this.hideNull).length < 1;
    }
    _renderRow(row, rownum) {
      return html`${(row || []).map((option, colnum) => html`
          <simple-picker-option
    ...
    _setSelectedOption() {
      ...
      if (this.options && this.options.length > 0) {
        this.__options = typeof this.options === "string" ? JSON.parse(this.options) : this.options;
