...
export class EvoToWc {
  ...
  convertExpandables(expandables = []) {
    expandables.forEach(expandable => {
      let collapse = globalThis.document.createElement("a11y-collapse"),
        ...
        content = collapseable ? [...expandable.querySelector(".collapseable").childNodes] : [],
  ...
  convertFigures(figures = []) {
    import("../a11y-figure/a11y-figure.js");
  ...
  convertThumbnails(images) {
    images.forEach(image => {
      ...
      gallery.innerHTML = `<figure><img src="${src}" ${thumb ? `` : ""}></figure>`, parent = image.parentElement;
  ...
  insertChart(table, type = "bar") {
    let newTable = globalThis.document.createElement("table"),
      chart = table.classList.contains(`${type}chart`) ? globalThis.document.createElement(`lrndesign-${type}`) : undefined;
    if (chart) {
      chart.scale = "ct-octave";
      [...table.childNodes].forEach(node => newTable.append(node.cloneNode(true)));
  ...
  slotGallery(item) {
    let figure = globalThis.document.createElement("figure"),
      ...
      siblings = item.querySelector("figcaption") ? [...item.querySelector("figcaption").childNodes] : [...item.childNodes],
...
globalThis.EvoToWc.requestAvailability = () => {
  ...
  globalThis.dispatchEvent(new CustomEvent("register-hax-converter", {
    bubbles: true,
    composed: true,
    cancelable: true,
