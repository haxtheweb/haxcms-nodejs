...
class BarcodeReader extends LitElement {
  ...
  constructor() {
    ...
    globalThis.ESGlobalBridge.requestAvailability().load("ZXing", decodeURIComponent(import.meta.url) + "/../lib/zxing.js");
    globalThis.addEventListener(`es-bridge-zxing-loaded`, this._control.bind(this));
  }
  _control() {
    ...
    globalThis.navigator.mediaDevices.enumerateDevices().then(gotDevices).then(getStream).catch(handleError);
    ...
    function getStream() {
      ...
      globalThis.navigator.mediaDevices.getUserMedia(constraints).then(gotStream).catch(handleError);
    }
  ...
  async start() {
    this.shadowRoot.querySelector("simple-icon-button").addEventListener("click", () => {
      if (this.shadowRoot.querySelector(".render").innerHTML === "Initialize") {
        this._control();
      }
    });
  }
