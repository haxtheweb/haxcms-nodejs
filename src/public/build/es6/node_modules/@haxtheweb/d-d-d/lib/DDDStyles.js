...
export function HAXOptionSampleFactory(type) {
  return Object.keys(ApplicationAttributeData[type]).map(key => {
    return {
      value: key,
      html: ["primary", "accent"].includes(type) ? html`<d-d-d-sample
            @click="${updatePreviewColorVar}"
            type="${type}"
            option="${key}"
          ></d-d-d-sample>` : html`<d-d-d-sample type="${type}" option="${key}"></d-d-d-sample>`
    };
  });
}
...
export const instructionalStyles = Object.keys(learningComponentColors).map(item => {
