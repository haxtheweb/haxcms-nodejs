import { html, css, render, unsafeCSS } from "../../lit/index.js";
import { SimpleColors } from "../simple-colors/simple-colors.js";
import { UndoManagerBehaviors } from "../undo-manager/undo-manager.js";
import { HAXStore } from "./lib/hax-store.js";
import { autorun, toJS } from "../../mobx/dist/mobx.esm.js";
import "./lib/hax-text-editor-toolbar.js";
import { encapScript, wipeSlot, generateResourceID, nodeToHaxElement, haxElementToNode, camelToDash, wrap, unwrap, ReplaceWithPolyfill, normalizeEventPath } from "../utils/utils.js";
...
import { SimpleIconsetStore } from "../simple-icon/lib/simple-iconset.js";
...
import { SuperDaemonInstance } from "../super-daemon/super-daemon.js";
...
class HaxBody extends I18NMixin(UndoManagerBehaviors(SimpleColors)) {
  ...
  _mouseMove(e) {
    if (this.editMode && HAXStore.ready) {
      ...
      this.__mouseQuickTimer = setTimeout(() => {
        if (this.__activeHover && this.__activeHover != eventPath[0].closest("[data-hax-ray]:not(li)")) {
          this.__activeHover = null;
        }
      }, 300);
      clearTimeout(this.__mouseTimer);
      this.__mouseTimer = setTimeout(() => {
        ...
        if (target) {
          ...
          if (!eventPath[0].closest("[data-move-order]:not(.has-nodes")) {
            ...
            this.__activeHover = eventPath[0].closest("[data-move-order]").parentNode.parentNode.host;
          } else {
            ...
            this.__slot = eventPath[0].closest("[data-move-order]").getAttribute("id").replace("col", "col-");
            ...
            this.__activeHover = eventPath[0].closest("[data-move-order]").parentNode.parentNode.host.children[0];
          }
  ...
  render() {
    return html`
      ...
      <absolute-position-behavior
        ...
        .target="${!this.activeNode ? globalThis.document.body : this.viewSourceToggle ? this.activeNode.parentNode : this.activeNode}"
  ...
  calcClasses(activeNode) {
    let txt = "not-text";
    if (activeNode && activeNode.getAttribute && !activeNode.getAttribute("data-hax-lock") && activeNode.parentNode && activeNode.parentNode.getAttribute && !activeNode.parentNode.getAttribute("data-hax-lock") && HAXStore.isTextElement(activeNode) && !HAXStore.isSingleSlotElement(activeNode)) {
      txt = "is-text";
    }
    return txt;
  }
  ...
  firstUpdated(changedProperties) {
    render(this.HAXBODYStyleSheetContent(), this.shadowRoot.querySelector("#hax-body-style-element"));
  ...
  _onKeyUp(e) {
    if (["ArrowUp", "ArrowDown"].includes(e.key) && this.activeNode && HAXStore.isTextElement(this.activeNode) && !SuperDaemonInstance.opened) {
      let key = e.key;
  ...
  _onKeyDown(e) {
    ...
    if (this.editMode && globalThis.document.activeElement.tagName !== "HAX-TRAY" && globalThis.document.activeElement.tagName !== "BODY" && globalThis.document.activeElement.tagName !== "SIMPLE-MODAL") {
        ...
        if (sel.anchorNode != null) {
          switch (e.key) {
            ...
            case "Delete":
              ...
              if (this.activeNode && this.activeNode.textContent == "" && this.activeNode.previousElementSibling && this.activeNode.previousElementSibling.tagName && (["TABLE", "EDITABLE-TABLE", "IFRAME-LOADER", "IFRAME", "WEBVIEW"].includes(this.activeNode.previousElementSibling.tagName) || this.activeNode.previousElementSibling.tagName === "PAGE-BREAK" && this.shadowRoot.querySelector("#body").assignedNodes({
                flatten: true
              }).length === 2 && this.shadowRoot.querySelector("#body").assignedNodes({
                flatten: true
              })[1] === this.activeNode)) {
                e.preventDefault();
              }
            ...
            case "/":
              const rng = HAXStore.getRange();
              if (this.activeNode && HAXStore.isTextElement(this.activeNode) && rng.commonAncestorContainer.textContent.trim() == "") {
                e.preventDefault();
                ...
                SuperDaemonInstance.runProgram(rng.commonAncestorContainer.textContent.trim(), "*");
  ...
  keyboardShortCutProcess(guess) {
    ...
    if (HAXStore.keyboardShortcuts[guess.replace(" ", "")]) {
      let el = haxElementToNode(HAXStore.keyboardShortcuts[guess.replace(" ", "")]);
  ...
  setActiveNode(node, force = false) {
    if (node && this.editMode && this.activeNode && (HAXStore.isTextElement(this.activeNode) || force)) {
  ...
  async canTansformNode(node = null) {
    return (await this.replaceElementWorkflow(node, true).length) > 0 ? true : false;
  }
  /**
  ...
  getAllSlotConfig(node) {
    ...
    return !!grid && !!grid.tag ? this.getSlotConfig(HAXStore.elementList[grid.tag], slot) : undefined;
  }
  /**
   *
  ...
  getSlotConfig(slotId = "", props = {}) {
    let settings = props.settings,
      matchingSlots = !!settings ? Object.keys(settings || {}).map(group => settings[group].filter(setting => !!setting.slot && (!slotId || setting.slot === slotId))).flat() : undefined;
  ...
  async replaceElementWorkflow(activeNode = null, testOnly = false) {
    ...
    if (typeof HAXStore.elementList[element.tag] !== typeof undefined && HAXStore.elementList[element.tag].gizmo !== false && typeof HAXStore.elementList[element.tag].gizmo.handles !== typeof undefined && HAXStore.elementList[element.tag].gizmo.handles.length > 0) {
    ...
    let slots = !!grid ? this.getSlotConfig(slot, HAXStore.elementList[grid.tag]) : undefined;
    let exclusions = !!grid && grid.tag === "grid-plate" ? ["grid-plate"] : !!slots ? slots.excludedSlotWrappers : undefined;
    let inclusions = !!slots ? slots.allowedSlotWrappers : undefined;
    if (!!exclusions || !!inclusions) haxElements = haxElements.filter(el => !exclusions.includes(el.tag) && (!inclusions || inclusions.includes(el.tag)));
    ...
    if (haxElements.length > 0) {
      ...
      if (typeof HAXStore.elementList[tag] !== typeof undefined && HAXStore.elementList[tag].gizmo !== false) {
        humanName = HAXStore.elementList[tag].gizmo.title;
      }
      if (!testOnly) {
        HAXStore.activePlaceHolder = this.activeNode;
        HAXStore.haxAppPicker.presentOptions(haxElements, "__convert", `Change ${humanName} to...`, "gizmo");
      }
    } else {
      if (!testOnly) {
   ...
   */
  haxInsert(tag, content, properties = {}, active = this.activeNode, child = false) {
    ...
    if (HAXStore.activePlaceHolder !== null && typeof HAXStore.activePlaceHolder.style !== typeof undefined) {
    ...
    for (var i = 0; i < children.length; i++) {
      ...
      // keep comments with a special case since they need wrapped
        ...
        // yet we need to remove from output as they do not really exist :)
            ...
            let tmp = await HAXStore.runHook(children[i], "activeElementChanged", [this.activeNode, false]);
  ...
  async haxDuplicateNode(node) {
    ...
    if (HAXStore.testHook(node, "preProcessInsertContent")) {
      haxElement = await HAXStore.runHook(node, "preProcessInsertContent", [haxElement, this.activeNode]);
    }
    ...
    if (nodeClone.tagName.toLowerCase() === "webview" && HAXStore._isSandboxed && typeof nodeClone.guestinstance !== typeof undefined) {
  ...
  positionContextMenus(node = this.activeNode) {
    ...
    if (node && node.tagName && this.ready && !this._useristyping) {
      ...
      if (HAXStore.elementList && HAXStore.elementList[tag] && HAXStore.elementList[tag].contentEditable && node.getAttribute("data-hax-lock") === null && node.parentNode && node.parentNode.getAttribute("data-hax-lock") === null) {
      ...
      this.__positionContextTimer = setTimeout(() => {
        if (!HAXStore._lockContextPosition) {
          ...
          if (typeof props !== typeof undefined && !HAXStore.isTextElement(node)) {
            // hide text
            this._hideContextMenu(this.contextMenus.text);
            props.element = node;
          } else {
          ...
          let gizmo = (props || {}).gizmo || {},
            inlineGizmo = props && props.gizmo && (props.gizmo.handles || []).filter(handle => (handle || {}).type === "inline").length > 0;
          if (!props || props.editingElement == "core") {
            ...
            if (this.activeNode && (this.activeNode.tagName === "LI" || this._HTMLInlineTextDecorationTest(this.activeNode))) {
    ...
    setTimeout(() => {
      ...
      while (fragment.firstChild !== null) {
        if (typeof fragment.firstChild.tagName !== typeof undefined) {
          ...
          if (HAXStore._isSandboxed && fragment.firstChild.tagName.toLowerCase() === "iframe") {
            // Create a replacement tag of the desired type
  ...
  sortGridSlots(grid = this.activeNode) {
    ...
    let slots = grid.tagName === "GRID-PLATE" ? grid.layout.split("-").map((col, i) => `col-${i}`) : HAXStore.slotsFromSchema(schema).map(slot => slot.slot);
    slots.reverse().forEach((slot, i) => {
      if (i == 0) {
        grid.querySelectorAll(`[slot=${slot}]`).forEach(node => grid.append(node));
      } else {
        [...grid.querySelectorAll(`[slot=${slot}]`)].reverse().forEach(node => grid.insertBefore(node, grid.firstChild));
      }
    });
  }
  /**
  ...
  async _haxContextOperation(e) {
    let detail = e.detail,
      ...
      slot = !eventPath || !eventPath[0] ? undefined : eventPath[0].getAttribute("data-slot");
    ...
    switch (detail.eventName) {
      ...
      case "insert-below-active":
        ...
        break;
      case "move-to-slot":
        if (slot && this.activeNode && HAXStore.isGridPlateElement(this.activeNode.parentNode)) {
          this.activeNode.slot = slot;
          this.sortGridSlots(this.activeNode.parentNode);
        }
        break;
      case "insert-into-active":
        if (slot && this.activeNode && HAXStore.isGridPlateElement(this.activeNode)) {
          let p = globalThis.document.createElement("p");
          p.slot = slot;
      ...
      case "hax-select-grid-item":
        if (eventPath[0] && eventPath[0].eventData) {
          let data = eventPath[0].eventData,
            ...
            //where to set focus
            focusTarget = invisible ? grid : [...grid.children].filter(child => !slot || slot === "" ? !child.slot || child.slot === "" : child.slot === slot)[i] || target,
      ...
      case "hax-edit-element-toggle":
        if (eventPath[0] && eventPath[0].eventData) {
          let data = eventPath[0].eventData;
          if (data && data.target && data.editMode) this.toggleElementEditMode(data.target, data.editMode);
        }
        break;
      case "hax-source-view-toggle":
        if (!this.activeNode.__haxSourceView) {
          ...
          if (this.activeNode.getAttribute && this.activeNode.getAttribute("slot") != null) {
            HAXStore.activeEditingElement.setAttribute("slot", this.activeNode.getAttribute("slot"));
          }
          this.__ignoreActive = true;
          ...
          if (HAXStore.testHook(this.activeNode, "preProcessNodeToContent")) {
            HAXStore.activeNode = await HAXStore.runHook(this.activeNode, "preProcessNodeToContent", [this.activeNode]);
          }
          ...
          let replacement = await HAXStore.runHook(HAXStore.activeEditingElement, "activeElementChanged", [this.activeNode, false]);
          let oldSchema = {};
          if (this.activeNode && this.activeNode.tagName) {
            oldSchema = HAXStore.haxSchemaFromTag(this.activeNode.tagName.toLowerCase());
          } else if (this.activeNode.parentElement && this.activeNode.parentElement.tagName) {
            oldSchema = HAXStore.haxSchemaFromTag(this.activeNode.parentElement.tagName.toLowerCase());
          }
          // test for slots to match to ensure this is maintained
      ...
      case "hax-full-text-editor-toggle":
        if (!this.activeNode.__haxSourceView) {
          this.activeNode.__haxSourceView = true;
          // could be 1st time this shows up so ensure we import
          import("../rich-text-editor/rich-text-editor.js").then(() => {
            HAXStore.activeEditingElement = globalThis.document.createElement("rich-text-editor");
            HAXStore.activeEditingElement.type = "rich-text-editor-toolbar-full";
            // test for slots to match to ensure this is maintained
            if (this.activeNode.getAttribute && this.activeNode.getAttribute("slot") != null) {
              HAXStore.activeEditingElement.setAttribute("slot", this.activeNode.getAttribute("slot"));
            }
          ...
          // run internal state hook if it exist and if we get a response
          let replacement = await HAXStore.runHook(HAXStore.activeEditingElement, "activeElementChanged", [this.activeNode, false]);
          let oldSchema = HAXStore.haxSchemaFromTag(this.activeNode.tagName.toLowerCase());
          // test for slots to match to ensure this is maintained
          if (this.activeNode && this.activeNode.getAttribute && this.activeNode.getAttribute("slot") != null) {
            replacement.setAttribute("slot", this.activeNode.getAttribute("slot"));
          }
      ...
      case "text-tag":
        ...
        HAXStore.activeNode = this.haxChangeTagName(this.activeNode, detail.value);
        this.positionContextMenus();
        break;
      case "text-tag-ul":
        // trigger the default selected value in context menu to match
  ...
  __focusLogic(target, autoFocus = true) {
    ...
    if (this.editMode && !this.__tabTrap) {
      ...
      if (containerNode.tagName === "SPAN" && HAXStore.isTextElement(containerNode.parentNode) && containerNode.parentNode.getAttribute("slot") == "") {
      ...
      else if (containerNode.tagName === "LI" && HAXStore.isTextElement(containerNode.parentNode) && ["UL", "OL"].includes(containerNode.parentNode.tagName)) {
          containerNode = target.parentNode;
        }
      let activeNode = null;
      ...
      if (this._validElementTest(containerNode) && containerNode.parentNode && containerNode.parentNode.tagName) {
        ...
        if (!activeNode.haxUIElement && !activeNode.classList.contains("ignore-activation")) {
          ...
          setTimeout(() => {
            if (autoFocus && !this.__mouseDown && HAXStore.isTextElement(activeNode)) {
              try {
                var range = globalThis.document.createRange();
                var sel = HAXStore.getSelection();
                range.setStart(this.activeNode, 0);
  ...
  async _editModeChanged(newValue, oldValue) {
    ...
    if (newValue) {
      ...
      globalThis.addEventListener("hax-context-item-selected", this._haxContextOperation, {
        signal: this.windowControllers.signal
      });
      globalThis.addEventListener("hax-toggle-active-node-lock", this._toggleNodeLocking, {
        signal: this.windowControllers.signal
      });
      ...
      this._observer = new MutationObserver(mutations => {
        var mutFind = false;
        if (!this.__ignoreActive && !this.__dragMoving && !this.undoStackIgnore && !this.__fakeEndCap && this.ready && this.editMode && this.shadowRoot) {
          mutations.forEach(mutation => {
            ...
            if (mutation.addedNodes.length > 0) {
              for (var node of mutation.addedNodes) {
                if (this._validElementTest(node)) {
                  ...
                  if (!this.__delHit && node.tagName === "BR" && node.parentElement && HAXStore.__validGridTags().includes(node.parentElement.tagName.toLowerCase()) && node === node.parentElement.childNodes[node.parentElement.childNodes.length - 1]) {
            ...
            // this way we can't delete... EVERYTHING
            else if (this.ready && this.editMode && HAXStore.ready && mutation.addedNodes.length === 0 && mutation.removedNodes.length > 0 && this.shadowRoot && (this.shadowRoot.querySelector("#body").assignedNodes({
                flatten: true
              }).length === 1 && this.shadowRoot.querySelector("#body").assignedNodes({
                flatten: true
              })[0].tagName === "PAGE-BREAK" || this.shadowRoot.querySelector("#body").assignedNodes({
                flatten: true
              }).length === 0)) {
                ...
                setTimeout(() => {
                  if (this.shadowRoot.querySelector("#body").assignedNodes({
                    flatten: true
                  }).length === 1 && this.shadowRoot.querySelector("#body").assignedNodes({
                    flatten: true
                  })[0].tagName === "PAGE-BREAK" || this.shadowRoot.querySelector("#body").assignedNodes({
  ...
  _validElementTest(node, allowInline = false) {
    ...
    if (!node.haxUIElement && node.tagName && !["TEMPLATE", "HAX-BODY", "FAKE-HAX-BODY-END"].includes(node.tagName)) {
      ...
      if (!allowInline && this._HTMLInlineTextDecorationTest(node) && node.parentNode != "HAX-BODY") {
        return false;
      }
      return true;
    }
  ...
  _HTMLInlineTextDecorationTest(node) {
    return ["span", "b", "strong", "i", "em", "u", "strike"].includes(node.tagName.toLowerCase());
  }
  /**
  ...
  __getLayoutOrder(target, layout) {
    ...
    let slot = target.getAttribute("slot"),
      ...
      containers = [...layout.shadowRoot.querySelectorAll("[slot]")].map(node => node.parentNode),
  ...
  async __sortLayoutChildren(layout) {
    ...
    try {
      ...
      let children = Array.prototype.reduce.call(layout.querySelectorAll("[slot]"), function (acc, e) {
        return acc;
      }, []);
  ...
  __layoutSlots(layout) {
    return layout.shadowRoot ? [...layout.shadowRoot.querySelectorAll("[slot]")].map(container => container.getAttribute("name")) : [];
  }
  __applyDragDropState(layout, haxRay) {
    ...
    if (HAXStore.isGridPlateElement(layout)) layout.setAttribute("data-hax-grid", true);
    ...
    if (haxRay && layout.shadowRoot) {
      ...
      slots.forEach(slot => slot.addEventListener("slotchange", events.slotchange));
          ...
          mutations.forEach(mutation => {
            ...
            mutation.addedNodes.forEach(node => {
              if (node.tagName && node !== this) {
                ...
                if (node.parentElement && node.parentElement.tagName !== "HAX-BODY" && !this.__layoutSlotValid(node, layout) && this.__layoutSlots(layout).length > 0) {
      ...
      slots.forEach(slot => slot.removeEventListener("slotchange", events.slotchange));
      layout.removeAttribute("data-hax-ray");
    }
  }
  __isLayout(el) {
    return el && HAXStore.haxSchemaFromTag(el.tagName) && HAXStore.haxSchemaFromTag(el.tagName).type === "grid";
  }
  /**
  ...
  dropEvent(e) {
    if (this.editMode) {
      ...
      try {
        ...
        if (HAXStore.__dragTarget === null && e.dataTransfer && e.dataTransfer.items && e.dataTransfer.items.length > 0 && e.dataTransfer.items[0].kind === "file") {
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
          ...
          if (target && this._validElementTest(target) && typeof target.focus === "function") {
            ...
            if (HAXStore.testHook(HAXStore.activeNode, "trayDragNDropToNode")) {
              HAXStore.runHook(HAXStore.activeNode, "trayDragNDropToNode", [HAXStore.activeNode]);
            }
  ...
  handleMousemove(e) {
    ...
    var documentWidth = Math.max(globalThis.document.body.scrollWidth, globalThis.document.body.offsetWidth, globalThis.document.body.clientWidth, globalThis.document.documentElement.scrollWidth, globalThis.document.documentElement.offsetWidth, globalThis.document.documentElement.clientWidth);
    var documentHeight = Math.max(globalThis.document.body.scrollHeight, globalThis.document.body.offsetHeight, globalThis.document.body.clientHeight, globalThis.document.documentElement.scrollHeight, globalThis.document.documentElement.offsetHeight, globalThis.document.documentElement.clientHeight);
  ...
  unsetSlotEditMode(node) {
    if (node.getAttribute && node.getAttribute) {
      let settings = !node.getAttribute("data-grid-saved-settings") ? undefined : JSON.parse(node.getAttribute("data-grid-saved-settings"));
  ...
  setSlotEditMode(node, settings, focusTarget) {
    let saved = !node.getAttribute("data-grid-saved-settings") ? {} : JSON.parse(node.getAttribute("data-grid-saved-settings")),
  ...
  async _activeNodeChanged(newValue, oldValue) {
    ...
    if (this.editMode && typeof newValue !== typeof undefined && newValue !== null && newValue.parentNode && newValue.tagName) {
      ...
      if ((HAXStore.isTextElement(newValue) || newValue.tagName === "HR" || HAXStore.isGridPlateElement(newValue)) && newValue.getAttribute("data-hax-lock") === null && newValue.parentNode.getAttribute("data-hax-lock") === null) {
    ...
    if (this.editMode && (await HAXStore.runHook(oldValue, "activeElementChanged", [oldValue, false]))) {
      this.__ignoreActive = true;
    }
    // attempt new value processing on element changed
    // @see haxHooks activeElementChanged
    if (this.editMode && (await HAXStore.runHook(newValue, "activeElementChanged", [newValue, true]))) {
      this.__ignoreActive = true;
    }
    // OLD VALUE TEST
    ...
    if (this.editMode && oldValue && oldValue.tagName) {
      ...
      if (oldSchema.editingElement != "core" || oldValue.parentNode && oldValue.parentNode.haxUIElement && oldValue.parentNode === HAXStore.activeEditingElement) {
        this.__ignoreActive = true;
        // run internal state hook if it exist and if we get a response
        let replacement = await HAXStore.runHook(oldValue.parentNode, "activeElementChanged", [oldValue, false]);
    ...
    if (this.editMode && newValue) {
      ...
      if (newSchema && newSchema.editingElement && newSchema.editingElement != "core") {
        ...
        if (newSchema.editingElement.import) {
          let basePath = new URL("./hax-body.js", import.meta.url).href + "/../../../";
        ...
        HAXStore.activeEditingElement = globalThis.document.createElement(newSchema.editingElement.tag);
        // test for slots to match to ensure this is maintained
        if (newValue.getAttribute && newValue.getAttribute("slot") != null) {
          HAXStore.activeEditingElement.setAttribute("slot", newValue.getAttribute("slot"));
        }
        ...
        await HAXStore.runHook(HAXStore.activeEditingElement, "activeElementChanged", [newValue, true]);
      }
    }
