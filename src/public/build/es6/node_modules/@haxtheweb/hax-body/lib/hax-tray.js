import { html, css } from "../../../lit/index.js";
import { winEventsElement, camelCaseToDash, wipeSlot, normalizeEventPath, localStorageSet } from "../../utils/utils.js";
import { HaxSchematizer, HaxElementizer } from "../../hax-body-behaviors/lib/HAXFields.js";
import { SimpleTourFinder } from "../../simple-popover/lib/SimpleTourFinder.js";
import { HAXStore } from "./hax-store.js";
import { autorun, toJS } from "../../../mobx/dist/mobx.esm.js";
import { HaxComponentStyles, HaxTrayDetail } from "./hax-ui-styles.js";
import "../../simple-fields/simple-fields.js";
import "../../simple-fields/lib/simple-fields-tab.js";
...
import { Undo } from "../../undo-manager/undo-manager.js";
import "../../iframe-loader/lib/loading-indicator.js";
import "../../simple-toolbar/lib/simple-toolbar-menu-item.js";
 ...
 * @element hax-tray
 */
class HaxTray extends I18NMixin(SimpleTourFinder(winEventsElement(SimpleColors))) {
  /**
   * Convention we use
   */
  static get tag() {
   ...
   */
  static get styles() {
    return [...(super.styles || []), ...HaxTrayDetail, ...HaxComponentStyles, css`
        :host {
  ...
  get trayStatus() {
    let status = this.collapsed ? "collapsed" : this.trayDetail == "view-source" ? "full-panel" : "side-panel";
    ...
    this.addEventListener("mousemove", e => {
      ...
      if (this.resizeDrag) {
        ...
        if (this.elementAlign === "right") {
          this.shadowRoot.querySelector(".detail").style.width = globalThis.innerWidth - this.__moveX - base + "px";
        } else {
          this.shadowRoot.querySelector(".detail").style.width = this.__moveX - base + "px";
        }
      }
    }, {
  ...
  get menuButtons() {
    return html`
      <hax-tray-button
        ...
        label="${this.elementAlign == "left" ? this.t.menuRight : this.t.menuLeft}"
        index="${this.elementAlign == "left" ? "1" : "0"}"
        tooltip="${this.t.move} ${this.elementAlign == "left" ? this.t.right : this.t.left}"
      >
      </hax-tray-button>
    `;
  }
  ...
  get contentEditTemplate() {
    return html` <simple-fields
      ...
      disable-responsive
      code-theme="${this.haxUiTheme == "system" ? "auto" : this.haxUiTheme == "haxdark" ? "vs-dark" : "vs"}"
  ...
  _refreshAddData() {
    this.shadowRoot.querySelector("#gizmobrowser").resetList(toJS(HAXStore.gizmoList));
    this.shadowRoot.querySelector("#staxbrowser").staxList = [...toJS(HAXStore.staxList)];
  }
  /**
  ...
  _processTrayEvent(e) {
    ...
    switch (evt) {
      ...
      case "insert-tag":
        ...
        if ((target.getAttribute("data-demo-schema") || e.detail.demoSchema) && schema && schema.demoSchema && schema.demoSchema[0]) {
          ...
          haxElement = HAXStore.haxElementPrototype(gizmo, properties, innerContent);
        }
      ...
      case "toggle-element-align":
        let directions = ["left", "right"],
          direction = !!directions[e.detail.index] ? directions[e.detail.index] : "left";
  ...
  startTour() {
    ...
    this.tourController = new AbortController();
    globalThis.addEventListener("tour-changed", this._handleTourChanged.bind(this), {
  ...
  firstUpdated(changedProperties) {
    ...
    if (!this.__setup) {
      this.shadowRoot.querySelector("#settingsform").schematizer = HaxSchematizer;
      this.shadowRoot.querySelector("#settingsform").elementizer = HaxElementizer;
      setTimeout(() => {
  ...
  async _setupForm() {
    ...
    if (activeNode && activeNode.tagName && HAXStore.elementList[activeNode.tagName.toLowerCase()]) {
      let props = {
        ...
        Object.keys(props.settings).forEach(propContainer => {
          ...
          props.settings[propContainer].forEach((el, i) => {
            ...
            if (["contenteditable", "data-hax-active", "data-hax-grid", "data-hax-layout"].includes(property)) {
              ...
              this.activeValue.settings[propContainer][property] = this.activeHaxElement.properties[property];
                  ...
                  if (collapseContainer.title && collapseContainer.properties) {
                    props.settings[propContainer][i].properties[collapseIndex].properties.forEach(nested => {
                      if (nested.property === property) {
                        ...
                        this.activeValue.settings[propContainer][el.property][collapseContainer.property][property] = this.activeHaxElement.properties[property];
                      }
      ...
      props.settings.developer.forEach((val, key) => {
        if (props.settings.developer[key].attribute) {
          props.settings.developer[key].property = props.settings.developer[key].attribute;
        }
        if (props.settings.developer[key].slot) {
          props.settings.developer[key].property = props.settings.developer[key].slot;
        }
      });
      ...
      let setProps = (propName, propTitle, settings = []) => {
        let filteredProps = !isGrid ? settings : settings.filter(prop => prop.slot !== "" && !prop.slot && (!prop.attribute || prop.attribute != "slot"));
  ...
  flattenObject = obj => {
    return Object.assign({}, ...function _flatten(o) {
      if (o) {
        return [].concat(...Object.keys(o).map(k => typeof o[k] === "object" && !Array.isArray(o[k]) ? _flatten(o[k]) : {
          [k]: o[k]
        }));
      }
      return o;
          ...
          for (let prop in settings[key]) {
            ...
            if (settings[key][prop] != null && !settings[key][prop].readOnly) {
              ...
              // prefix is a special attribute and must be handled this way
                  ...
                  this.dispatchEvent(new CustomEvent("hax-toggle-active-node-lock", {
                    bubbles: true,
                    composed: true,
                    cancelable: true,
                    detail: {
                  ...
                  // then by seeing if we have an array / object
                      ...
                      for (let propTmp in this.__activePropSchema.settings[key]) {
                        if (this.__activePropSchema.settings[key][propTmp].slot == prop) {
                          ...
                          this.__activePropSchema.settings[key][propTmp].allowedSlotWrappers && this.__activePropSchema.settings[key][propTmp].allowedSlotWrappers[0]) {
                            slotTag = this.__activePropSchema.settings[key][propTmp].allowedSlotWrappers[0];
                          } else if (this.activeNode.tagName.toLowerCase() === "code-editor") {
                            slotTag = "template";
                          } else {
                            //selects wrapper that is not excluded
                            let wrappers = ["span", "div", "p"],
                              exclusions = this.__activePropSchema.settings[key][propTmp].excludedSlotWrappers || [];
                            if (exclusions) wrappers = wrappers.filter(wrapper => !exclusions.includes(wrapper));
                          }
                          var tmpel = globalThis.document.createElement(slotTag);
                          if (this.__activePropSchema.settings[key][propTmp].slotAttributes) {
                            for (let attr in this.__activePropSchema.settings[key][propTmp].slotAttributes) {
  ...
  __lockAllSettings(status) {
    this.shadowRoot.querySelectorAll("simple-fields-tab *[is-simple-field-type]:not([name='settings.advanced.data-hax-lock'])").forEach(node => {
