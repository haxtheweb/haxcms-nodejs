...
import { MtzFileDownloadBehaviors } from "../../dl-behavior/dl-behavior.js";
import { stripMSWord, formatHTML, b64toBlob } from "../../utils/utils.js";
...
import "../../simple-toolbar/lib/simple-toolbar-menu.js";
...
class HaxViewSource extends I18NMixin(MtzFileDownloadBehaviors(LitElement)) {
  ...
  render() {
    return html`${this.hidden ? `` : html` <hax-toolbar>
            <hax-tray-button
            ...
            ${MicroFrontendRegistry.has("@core/prettyHtml") ? html`
                  <hax-tray-button
            ...
            ${HAXStore.revisionHistoryLink ? html` <hax-tray-button
          ...
          <div id="wrapper">
            ...
            <code-editor
              id="textarea"
              theme="${this.haxUiTheme == "hax" ? "vs" : this.haxUiTheme == "haxdark" ? "vs-dark" : "auto"}"
              language="html"
              font-size="13"
              word-wrap
            ></code-editor>
  ...
  async downloadDOCX(e) {
    import("../../file-system-broker/lib/docx-file-system-broker.js").then(async e => {
      let body = await HAXStore.activeHaxBody.haxToContent();
      globalThis.DOCXFileSystemBroker.requestAvailability().HTMLToDOCX(body, globalThis.document.title);
      HAXStore.toast(this.t.fileDownloaded);
      this.close();
    });
  }
  ...
  async downloadDOCXviaMicro(e) {
    ...
    if (response.status == 200) {
      ...
      link.href = globalThis.URL.createObjectURL(b64toBlob(`${response.data}`, "application/vnd.openxmlformats-officedocument.wordprocessingml.document"));
  ...
  async downloadMDviaMicro(e) {
    ...
    if (response.status == 200 && response.data) {
      const link = globalThis.document.createElement("a");
      // click link to download file
      link.href = globalThis.URL.createObjectURL(b64toBlob(btoa(response.data), "text/markdown"));
  ...
  async updateBodyFromHTML(e) {
    ...
    let children = HAXStore.activeHaxBody.shadowRoot.querySelector("#body").localName === "slot" ? HAXStore.activeHaxBody.shadowRoot.querySelector("#body").assignedNodes({
      flatten: true
    }) : [];
  ...
  openSource() {
    ...
    if (!globalThis.customElements.get("code-editor")) {
      import("../../code-editor/code-editor.js").then(() => {
        this.updateEditor();
  ...
  updateEditor() {
    if (HAXStore.activeHaxBody && this.shadowRoot && this.shadowRoot.querySelector("#textarea")) {
      this.shadowRoot.querySelector("#textarea").editorValue = "";
      setTimeout(async () => {
        const htmlCode = formatHTML(await HAXStore.activeHaxBody.haxToContent());
        this.shadowRoot.querySelector("#textarea").editorValue = htmlCode;
      }, 0);
    }
  }
