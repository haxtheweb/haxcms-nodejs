...
import { SimpleTourManager } from "../../simple-popover/lib/simple-tour.js";
import { HaxSchematizer, HaxElementizer } from "../../hax-body-behaviors/lib/HAXFields.js";
import { winEventsElement, getRange, stripMSWord, nodeToHaxElement, haxElementToNode, validURL, camelToDash, htmlEntities, localStorageGet, localStorageSet } from "../../utils/utils.js";
import { observable, makeObservable, computed, configure, autorun, toJS } from "../../../mobx/dist/mobx.esm.js";
configure({
  enforceActions: false
}); // strict mode off
import { HAXElement } from "../../hax-body-behaviors/hax-body-behaviors.js";
import { I18NMixin, I18NManagerStore } from "../../i18n-manager/lib/I18NMixin.js";
import { enableServices } from "../../micro-frontend-registry/lib/microServices.js";
import { SuperDaemonInstance } from "../../super-daemon/super-daemon.js";
import "../../media-behaviors/media-behaviors.js";
import "../../editable-table/editable-table.js";
import "../../iframe-loader/iframe-loader.js";
import "../../hax-iconset/lib/hax-iconset-manifest.js";
import { UserScaffoldInstance } from "../../user-scaffold/user-scaffold.js";
import "./hax-app.js";
...
class HaxStore extends I18NMixin(winEventsElement(HAXElement(LitElement))) {
   ...
   */
  insertLogicFromValues(values, context, failOnAnything = false, linkOnMultiple = false) {
    ...
    if (haxElements.length > 0) {
      ...
      if (haxElements.length === 1 || linkOnMultiple) {
        if (haxElements.length === 1 && typeof haxElements[0].tag !== typeof undefined) {
  ...
  guessGizmoType(guess) {
    if (typeof guess.source !== typeof undefined) {
      ...
      if (source.indexOf(".mp3") != -1 || source.indexOf(".midi") != -1 || source.indexOf(".mid") != -1) {
        return "audio";
      } else if (source.indexOf(".png") != -1 || source.indexOf(".jpg") != -1 || source.indexOf(".jpeg") != -1) {
        return "image";
   ...
   */
  toast(message, duration = 2000, extras = {}, classStyle = "capsule", closeText = this.t.close, eventCallback = null, slot = null) {
    // gets it all the way to the top immediately
    globalThis.dispatchEvent(new CustomEvent(this.toastShowEventName, {
      bubbles: true,
      composed: true,
      cancelable: true,
      detail: {
  ...
  isTextElement(node) {
    ...
    if (tag && this.validTagList.includes(tag)) {
      if (["p", "ol", "ul", "li", "a", "h1", "h2", "h3", "h4", "h5", "h6", "strike", "u", "b", "sub", "sup", "span", "mark", "abbr", "i", "bold", "em", "strong", "blockquote", "code", "figure"].includes(tag)) {
        return true;
      }
    }
    return false;
  ...
  isInlineElement(node) {
    ...
    if (tag && this.validTagList.includes(tag)) {
      if (this.haxSchemaFromTag(tag) && this.haxSchemaFromTag(tag).meta && this.haxSchemaFromTag(tag).meta.inlineOnly || ["a", "strike", "u", "b", "sub", "sup", "span", "code", "mark", "abbr", "i", "bold", "em", "strong"].includes(tag)) {
        return true;
      }
    }
    return false;
  ...
  isLayoutElement(node) {
    let schema = !!node && !!node.tagName ? this.haxSchemaFromTag(node.tagName) || {} : {};
  ...
  async _handleDynamicImports(items, haxAutoloader) {
    let basePath = new URL("./hax-store.js", import.meta.url).href + "/../../../../";
    ...
    for (let i in items) {
      ...
      if (globalThis.customElements.get(i)) {
        ...
        await import(importPath).then(response => {
          // see if it imported now
          if (globalThis.customElements.get(i) && globalThis.customElements.get(i).haxProperties) {
            this.setHaxProperties(globalThis.customElements.get(i).haxProperties, i);
          } else {
            // edge case of no definition
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (["haxAutoloader", "haxTray", "haxCancel"].includes(propName)) {
        ...
        this.__storeReady = setTimeout(() => {
          this._storePiecesAllHere(this.haxAutoloader, this.activeHaxBody, this.haxTray, this.haxCancel);
        }, 0);
      }
    });
  }
  ...
  firstUpdated(changedProperties) {
    ...
    let haxConfirm = sessionStorageGet("haxConfirm") || localStorageGet("haxConfirm");
    if (!haxConfirm) {
    ...
    `;
      ...
      if (sessionStorageGet("haxConfirm") && !localStorageGet("haxConfirm")) {
        ...
        try {
          let globalData = sessionStorageGet("haxUserData") ? JSON.parse(sessionStorageGet("haxUserData")) : {};
  ...
  _storePiecesAllHere(haxAutoloader, activeHaxBody, haxTray, haxCancel) {
    if (!this.ready && activeHaxBody && haxAutoloader && haxTray && haxCancel) {
      ...
      if (rtep) {
        rtep.shadowRoot.querySelector("#formfields").schematizer = HaxSchematizer;
        rtep.shadowRoot.querySelector("#formfields").elementizer = HaxElementizer;
      }
      ...
      if (haxTray.shadowRoot.querySelector("#haxcancelbutton")) {
        ...
        if (!!haxCancel.shadowRoot.querySelector("#dialog")) {
          globalThis.addEventListener("simple-modal-confirmed", this._handleConfirmCancel.bind(this));
        }
      }
      this.ready = true;
  ...
  async _onPaste(e) {
    if (this.editMode && globalThis.document.activeElement.tagName !== "HAX-TRAY" && globalThis.document.activeElement.tagName !== "BODY" && globalThis.document.activeElement.tagName !== "SIMPLE-MODAL") {
      ...
      //remove styling
      pasteContent = pasteContent.replace(/(?:style="(\S+:\s*[^;"]+;\s*)*)+"/g, "");
  ...
  __validTags() {
    return ["p", "div", "span", "mark", "abbr", "table", "caption", "sup", "sub", "u", "strike", "tr", "th", "td", "ol", "ul", "li", "a", "strong", "kbd", "tt", "em", "i", "b", "hr", "h1", "h2", "h3", "h4", "h5", "h6", "blockquote", "code", "figure", "figcaption", "img", "iframe", "video", "audio", "section", "dl", "dt", "dd", "template", "webview"];
  }
  /**
   * Types that we deem as valid
   */
  __validGizmoTypes() {
    return ["data", "video", "audio", "text", "link", "file", "pdf", "image", "csv", "doc", "document", "archive", "markdown", "html", "content", "text", "gif", "inline", "*"];
  }
  /**
  ...
  constructor() {
    ...
    if (typeof globalThis.speechSynthesis !== "undefined" && (globalThis.SpeechRecognition || globalThis.webkitSpeechRecognition || globalThis.mozSpeechRecognition || globalThis.msSpeechRecognition || globalThis.oSpeechRecognition)) {
    ...
    SuperDaemonInstance.defineOption({
      ...
      value: {
        ...
        program: async (input, values) => {
          ...
          await Object.keys(globalThis.SimplePickerEmojis).forEach(async category => {
            await globalThis.SimplePickerEmojis[category].forEach(async emoji => {
    ...
    SuperDaemonInstance.defineOption({
      ...
      value: {
        ...
        program: async (input, values) => {
          let results = [];
          let txt = globalThis.document.createElement("textarea");
          await Object.keys(globalThis.SimplePickerSymbols).forEach(async category => {
            await globalThis.SimplePickerSymbols[category].forEach(async symbol => {
...
`;
    ...
    globalThis.open(`https://github.com/haxtheweb/issues/issues/new?assignees=&labels=${tags}&template=issue-report.md&title=${title}&body=${encodeURIComponent(body)}`, "_blank");
  }
  ...
  _buildPrimitiveDefinitions() {
    ...
    let ahref = {
      ...
      gizmo: {
        ...
        icon: "icons:link",
        color: "blue-grey",
        tags: ["Resource", "link", "a", "url", "html", "href", "address", "http"],
        handles: [],
        meta: {
          author: "W3C"
        }
      ...
      settings: {
        ...
        developer: [{
          ...
          attribute: "rel",
          title: "rel",
          description: "Specifies the relationship between this document and the opened globalThis.document. Change as part of security or SEO policy.",
          inputMethod: "select",
          options: {
            noopener: "noopener",
    ...
    let table = {
      ...
      gizmo: {
        ...
        icon: "image:grid-on",
        color: "blue-grey",
        tags: ["Instructional", "table", "data", "html", "grid", "matrix", "spreadsheet", "csv", "excel"],
        meta: {
          hidden: true,
          author: "W3C"
        }
    ...
    let iframe = {
      ...
      gizmo: {
        ...
        icon: "hax:iframe",
        color: "blue-grey",
        tags: ["Resource", "iframe", "content", "url", "link", "embed", "https", "html", "address"],
        handles: [],
        meta: {
          author: "W3C",
          hidden: true
    ...
    for (let tag in this.__primsBuilder) {
      ...
      this.setHaxProperties({
        ...
        gizmo: {
          ...
          tags: this.__primsBuilder[tag].tags || ["Text", tag, "html", "text"],
    ...
    if (e.detail) {
      ...
      if (globalThis.customElements.get(details.tag)) {
        ...
        if (this.testHook(prototypeNode, "preProcessInsertContent")) {
          details = await this.runHook(prototypeNode, "preProcessInsertContent", [details, this.activeNode]);
        }
      }
      var properties = {};
      ...
      } else if (this.activeNode && this.activeNode.parentNode && this.activeNode.parentNode.tagName != "HAX-BODY") {
        let node = haxElementToNode({
          tag: details.tag,
          content: details.content,
          properties: properties
  ...
  slotsFromSchema(schema, optionalOnly = false) {
    ...
    return Object.keys({
      ...(settings || {})
    }).map(setting => (settings[setting] || []).filter(prop => {
  ...
  _superDaemonInsert(e) {
    if (SuperDaemonInstance.programTarget && e.detail.properties && (e.detail.properties.src || e.detail.properties.source || e.detail.properties.href)) {
      SuperDaemonInstance.programTarget.value = e.detail.properties.src || e.detail.properties.source || e.detail.properties.href;
    } else {
  ...
  _haxStoreRegisterApp(e) {
    if (e.detail) {
      ...
      if (app.connection && app.connection.protocol && app.connection.url) {
        ...
        preconnectlink.href = app.connection.protocol + "://" + app.connection.url;
      ...
      if (slotnodes.length > 0) {
        ...
        for (let j = 0, len2 = slotnodes.length; j < len2; j++) {
          if (typeof slotnodes[j].tagName !== typeof undefined) {
            ...
            // even though it is...
            if (!this.HTMLPrimativeTest(slotnodes[j]) && slotnodes[j].tagName !== "TEMPLATE") {
  ...
  async refreshActiveNodeForm() {
    this.haxTray.activeHaxElement = await nodeToHaxElement(this.haxTray.activeNode, null);
  ...
  async _haxStoreRegisterProperties(e) {
    if (e.detail && e.detail.properties && e.detail.tag) {
      ...
      if (!this.elementList[e.detail.tag]) {
        let detail = e.detail;
        detail.properties = await this.attemptGizmoTranslation(detail.tag, detail.properties);
        ...
        if (detail.properties.type == "grid" && !this.validGridTagList.find(element => {
          return element === detail.tag;
        })) {
        ...
        if (globalThis.customElements.get(e.detail.tag) && this.testHook(globalThis.document.createElement(e.detail.tag), "gizmoRegistration")) {
          await this.runHook(globalThis.document.createElement(e.detail.tag), "gizmoRegistration", [this]);
        }
      }
  ...
  get activeNodeIndex() {
    ...
    if (this.activeNode) {
      Array.from(toJS(this.activeHaxBody).children).map((el, i) => {
        if (toJS(this.activeNode) === el || toJS(this.activeNode).parentElement === el) {
          nodeLookup = i;
        }
      });
    }
  ...
  async attemptGizmoTranslation(tag, properties) {
    ...
    var translationMap = await I18NManagerStore.loadNamespaceFile(tag + ".haxProperties");
    // if we have a map, rewrite the matching properties within the objects
    if (!translationMap && this.globalPreferences.haxLang == FALLBACK_LANG && globalThis.customElements.get(tag) && globalThis.customElements.get(tag).haxProperties) {
