  ...
  return class extends SimpleTourFinder(SuperClass) {
    ...
    constructor() {
      ...
      autorun(() => {
        ...
        if (activeNode && activeNode.tagName) {
          ...
          this.parentSchema = activeNode && activeNode.parentNode ? HAXStore.haxSchemaFromTag(activeNode.parentNode.tagName) : undefined;
    ...
    get slotSchema() {
      ...
      if (this.activeNode && this.parentSchema) {
        ...
        if (!this.activeNode || !this.activeNode.parentNode) schema = HAXStore.schemaBySlotId(this.activeNode.parentNode, slot);
      }
      return schema;
    }
     ...
     */
    get layoutElement() {
      return this.activeNode && HAXStore.isLayoutElement(this.activeNode) ? this.activeNode : this.activeNode && this.activeNode.parentNode && HAXStore.isLayoutElement(this.activeNode.parentNode) ? this.activeNode.parentNode : undefined;
    }
    /**
     ...
     */
    get layoutParent() {
      return this.layoutElement && this.layoutElement.parentNode && HAXStore.isLayoutElement(this.layoutElement.parentNode) && this.layoutElement.parentNode.tagName && this.layoutElement.parentNode.tagName !== "HAX-BODY" ? this.layoutElement.parentNode : undefined;
    }
    /**
     ...
     */
    elementIcon(node) {
      let gizmo = node && this.elementGizmo(node) ? this.elementGizmo(node) : undefined;
     ...
     */
    elementGizmo(node) {
      let schema = node && node.tagName ? HAXStore.haxSchemaFromTag(node.tagName) : undefined;
    ...
    getFilteredBlocks(blocks = []) {
      return blocks.filter(block => {
        if (!block.tag) return;
        let tag = block.tag || "",
          wrapper = !!this.slotSchema && !!this.slotSchema.slotWrapper && !!this.slotSchema.slotWrapper ? this.slotSchema.slotWrapper : undefined,
          allowed = !!this.slotSchema && !!this.slotSchema.slotWrapper && !!this.slotSchema.allowedSlotWrappers ? this.slotSchema.allowedSlotWrappers : undefined,
          excluded = !!this.slotSchema && !!this.slotSchema.slotWrapper && !!this.slotSchema.excludedSlotWrappers ? this.slotSchema.excludedSlotWrappers : undefined,
    ...
    updated(changedProperties) {
      ...
      changedProperties.forEach((oldValue, propName) => {
        if (propName === "activeNode" && this.activeNode !== oldValue) this.setTarget(this.activeNode);
      });
    }
    setTarget(node = this.activeNode) {
      if (super.setTarget) super.setTarget(node);
      this.parentSchema = node && node.parentNode ? HAXStore.haxSchemaFromTag(node.parentNode.tagName) : undefined;
    }
  };
};