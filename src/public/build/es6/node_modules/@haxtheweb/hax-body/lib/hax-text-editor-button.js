...
import { RichTextEditorPromptButtonBehaviors } from "../../rich-text-editor/lib/buttons/rich-text-editor-prompt-button.js";
 ...
 */
class HaxTextEditorButton extends RichTextEditorPromptButtonBehaviors(LitElement) {
  /**
  ...
  updateElement() {
    ...
    this.fields = [...(settings.inline || settings.configure || []).map(f => {
  ...
  getValue() {
    ...
    this.fields.forEach(field => {
      if (field.property && field.property !== "innerHTML") val[field.property] = this.targetedNode && this.targetedNode.getAttribute ? this.targetedNode.getAttribute(field.property) : undefined;
      if (field.slot && field.slot !== "") this.targetedNode && this.targetedNode.querySelector && this.targetedNode.querySelector(`[slot=${field.slot}]`) ? this.targetedNode.querySelector(`[slot=${field.slot}]`).innerHTML : undefined;
      if (field.slot && field.slot === "") this.targetedNode && [...(this.targetedNode.children || [])].filter(`:not([slot])`) && [...(this.targetedNode.children || [])].filter(`:not([slot])`).length > 0 ? `${[...(this.targetedNode.children || [])].filter(`:not([slot])`).join("\n")}` : undefined;
    });
    return val;
  }
  setToggled() {
  ...
  updateSelection() {
    let tag = globalThis.document.createElement(this.tagsList),
      html = "";
    this.fields.forEach(field => {
      if (!!field.property && field.property !== "innerHTML" && field.property !== "innerText") tag[field.property] = this.value[field.property];
      if (!!field.slot && !!this.value[field.slot]) html += `<${this.getSlotWrapper(field)}${Object.keys(field.slotAttributes || {}).map(attr => ` ${attr}="${field.slotAttributes[attr]}"`)}>
            ${this.value[slot]}
  ...
  getSlotWrapper(field) {
    let fallback = field.slotWrapper,
      ...
      filter = ["span", "div", "p"].filter(wrapper => !excluded.includes(wrapper));
