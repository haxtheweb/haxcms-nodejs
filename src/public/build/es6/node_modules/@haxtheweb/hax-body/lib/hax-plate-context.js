import { LitElement, css, html } from "../../../lit/index.js";
...
import { wipeSlot } from "../../utils/utils.js";
...
class HaxPlateContext extends I18NMixin(HaxContextBehaviors(LitElement)) {
    ...
    return html`
      <hax-toolbar always-expanded>
        <div class="group">
          ...
          >
            ...
            ${!this.activeNode || !this.activeNode.parentNode || !HAXStore.isLayoutElement(this.activeNode.parentNode) ? "" : (this.slottedItems || []).map((slot, i) => html`
                      <simple-toolbar-menu-item
                        slot="menuitem"
          ...
          >
            ...
            ${(this.slottedItems || []).map((slot, i) => (slot.items || []).map((item, j) => html`
                  <simple-toolbar-menu-item
                    slot="menuitem"
                    class="hax-select-grid-item ${i < 1 && j < 1 ? "first-slot" : ""}"
                  >
                    <hax-context-item
                      action
                      align-horizontal="left"
                      ?disabled="${this.viewSource || this.activeNode === item}"
                      label="${slot.title || slot.id}${slot.items.length > 1 ? ` (${j + 1})` : ""}"
      ...
      editMode: slot.editMode
          ...
          >
            ...
            ${!HAXStore.isLayoutElement(this.activeNode) ? "" : (this.slottedItems || []).map((slot, i) => html`
                      <simple-toolbar-menu-item
                        slot="menuitem"
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (propName === "activeNode" || propName === "activeEditingElement") {
        ...
        this.editElementProperty = this.activeSchema && this.activeSchema.editElement ? this.activeSchema.editElement : false;
        this.editElementToggled = this.activeNode && this.editElementProperty && !!this.activeNode[this.editElementProperty];
      }
    });
  }
  ...
  _dragStart(e) {
    ...
    HAXStore.activeHaxBody._hideContextMenu(HAXStore.activeHaxBody.contextMenus.add);
    if (e.dataTransfer) {
