...
import { HAXElement, HAXWiring } from "../../hax-body-behaviors/hax-body-behaviors.js";
        ...
        try {
          ...
          if (typeof effectiveChildren[i].getHaxProperties === "function") {
            ...
            if (!globalThis.customElements.get(name)) {
              ...
              if (globalThis.WCAutoload && globalThis.WCAutoload.requestAvailability() && globalThis.WCAutoload.requestAvailability().registry.getPathToTag(name)) {
                fileLocation = globalThis.WCAutoload.requestAvailability().registry.getPathToTag(name);
              } else {
                // fallback support since we now support import / a complex object
                let nameLocation = varGet(HAXStore, `__appStoreData.autoloader.${name}.import`, varGet(HAXStore, `__appStoreData.autoloader.${name}`, `@haxtheweb/${name}/${name}.js`));
                fileLocation = `${new URL("./hax-autoloader.js", import.meta.url).href + "/../../../../"}${nameLocation}`;
              }
              import(fileLocation).then(response => {
                ...
                // appstore definition
                else if (varGet(HAXStore, `__appStoreData.autoloader.${name}.haxProperties`, false)) {
                    this.setHaxProperties(varGet(HAXStore, `__appStoreData.autoloader.${name}.haxProperties`, false), name);
                  } else {
                    console.warn(`${name} didn't have hax wiring so HAX guessed as best it can. See https://haxtheweb.org/documentation-1/hax-development/hax-schema for documentation on adding custom wiring for better UX.`);
