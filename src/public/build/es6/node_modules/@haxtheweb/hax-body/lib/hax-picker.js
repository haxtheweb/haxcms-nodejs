...
class HaxPicker extends LitElement {
  ...
  render() {
    return html`
      ${!!this.keywords ? html` <div id="filters">
            <simple-icon-button-lite
      ...
      <simple-button-grid columns="4" always-expanded>
        ${this.selectionList.map((element, index) => !this._isFiltered(element.keywords) ? "" : html`
                ...
                >
                  <hax-tray-button
                    ...
                    data-selected="${index}"
                    ?disabled="${HAXStore.activeGizmo && HAXStore.activeGizmo.tag == element.tag}"
                    label="${element.title}"
                    icon="${element.icon}"
                    icon-position="top"
                    slot="button"
  ...
  _handleFilters(e) {
    let filters = this.shadowRoot && this.shadowRoot.querySelector("#hax-gizmo-filters") ? this.shadowRoot.querySelector("#hax-gizmo-filters") : undefined;
