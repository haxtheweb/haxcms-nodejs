...
const ChartistRenderSuper = function (SuperClass) {
  return class extends SuperClass {
    ...
    _loadScripts(classname, path, fnc = this._getChart) {
      let basePath = new URL("./chartist-render.js", import.meta.url).href + "/../";
      let location = `${basePath}${path}`;
      globalThis.addEventListener(`es-bridge-${classname}-loaded`, fnc.bind(this), {
    ...
    _renderTable() {
      ...
      if (data) {
        let rowHeads = data[1] && data[1][0] && isNaN(data[1][0]),
          ...
          tbody = data.series ? data.series : data.slice(thead ? 1 : 0, data.length).map(row => rowHeads ? {
            th: row[0],
