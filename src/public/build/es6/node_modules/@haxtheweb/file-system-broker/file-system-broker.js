...
class FileSystemBroker extends HTMLElement {
  ...
  async openDir(recursive = true, options = {}) {
    ...
    this.files = [];
    this.files = await this.__readDir(this.dirHandler, recursive, this.dirHandler.name || "", this.dirHandler);
    return this.files;
  }
  ...
  async writeFileInDir(fileName, content = "", options = {}) {
    try {
      ...
      for await (const entry of this.dirHandler.values()) {
        if (fileName && typeof entry.getFile === "function" && entry.name === fileName) {
          var FileSystemFileHandle = await this.dirHandler.getFileHandle(entry.name);
    ...
    for await (let [name, handle] of dirHandle) {
      ...
      if (handle.kind === "directory") {
        ...
        if (name !== ".git" && recursive) {
          files.push(...(await this.__readDir(handle, recursive, folder + "/" + name, handle)));
        }
      } else {
        files.push({
          name,
...
globalThis.FileSystemBroker.requestAvailability = () => {
  ...
  if (!globalThis.FileSystemBroker.instance && globalThis.document && globalThis.document.body) {
    globalThis.FileSystemBroker.instance = globalThis.document.createElement("file-system-broker");
    globalThis.document.body.appendChild(globalThis.FileSystemBroker.instance);
  }
  return globalThis.FileSystemBroker.instance;
};
// forces appending
const FileSystemBrokerSingleton = globalThis.FileSystemBroker.requestAvailability();
