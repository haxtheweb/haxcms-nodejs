...
class DOCXFileSystemBroker extends FileSystemBroker {
  ...
  constructor() {
    super();
    this.libPath = new URL("./docx-file-system-broker.js", import.meta.url).href + "/../";
    this.libPath += "mammoth/";
    ESGlobalBridgeStore.load("mammoth", this.libPath + "mammoth.browser.min.js").then(() => {
      if (globalThis.mammoth) {
        this.docx = globalThis.mammoth;
        // fire event in case anyone wants to react on loaded event
        this.dispatchEvent(new CustomEvent("docx-reader-ready", {
          bubbles: true,
          composed: true,
          cancelable: false,
          detail: this
  ...
  __toHTML(buffer, name) {
    ...
    globalThis.dispatchEvent(new CustomEvent("docx-file-system-data", {
      composed: false,
      bubbles: false,
      cancelable: true,
      detail: {
  ...
  HTMLToDOCX(content, filename, dl = true) {
    ...
    if (dl) {
      ...
      dl.href = `data:application/vnd.ms-word;charset=utf-8,` + encodeURIComponent(fileContents);
...
globalThis.DOCXFileSystemBroker.requestAvailability = () => {
  ...
  if (!globalThis.DOCXFileSystemBroker.instance) {
    globalThis.DOCXFileSystemBroker.instance = globalThis.document.createElement("docx-file-system-broker");
...
// forces appending
const DOCXFileSystemBrokerSingleton = globalThis.DOCXFileSystemBroker.requestAvailability();
export { DOCXFileSystemBrokerSingleton, DOCXFileSystemBroker, FileSystemBroker, FileSystemBrokerSingleton };