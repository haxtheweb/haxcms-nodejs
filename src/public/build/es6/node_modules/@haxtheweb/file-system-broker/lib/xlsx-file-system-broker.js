...
class XLSXFileSystemBroker extends FileSystemBroker {
  ...
  constructor() {
    ...
    this.libPath = new URL("./xlsx-file-system-broker.js", import.meta.url).href + "/../";
    this.libPath += "xlsx/";
    ESGlobalBridgeStore.load("xlsx", this.libPath + "dist/xlsx.full.min.js").then(() => {
      if (globalThis.XLSX) {
        this.XLSX = globalThis.XLSX;
        // fire event in case anyone wants to react on loaded event
        this.dispatchEvent(new CustomEvent("xlsx-ready", {
          bubbles: true,
          composed: true,
          cancelable: false,
          detail: this
  ...
  workbookFromJSON(data) {
    ...
    for (const i in data) {
      let ws = this.XLSX.utils.json_to_sheet(data[i], {
        skipHeader: true
      });
    ...
    worker.onmessage = e => {
      switch (e.data.t) {
        ...
        case "e":
          ...
          break;
        case this.XW.msg:
          globalThis.dispatchEvent(new CustomEvent("xlsx-file-system-data", {
            composed: false,
            bubbles: false,
            cancelable: true,
            detail: {
              ...
              format: format,
              operation: op,
              data: this.processWorker(JSON.parse(e.data.d), format, filename)
            }
          }));
          break;
      }
...
globalThis.XLSXFileSystemBroker.requestAvailability = () => {
  ...
  if (!globalThis.XLSXFileSystemBroker.instance) {
    globalThis.XLSXFileSystemBroker.instance = globalThis.document.createElement("xlsx-file-system-broker");
...
// forces appending
const XLSXFileSystemBrokerSingleton = globalThis.XLSXFileSystemBroker.requestAvailability();
export { XLSXFileSystemBrokerSingleton, XLSXFileSystemBroker, FileSystemBroker, FileSystemBrokerSingleton };