export class PageBreakManagerEl extends HTMLElement {
    ...
    if (this.target) {
      if (rel === "indent") {
        ...
        if (this.target.querySelector(`page-break[path="${el.parent}"]`)) {
          targetNode = this.target.querySelector(`page-break[path="${el.parent}"]`);
          // get parent of parent
          if (this.target.querySelector(`page-break[path="${targetNode.parent}"]`)) {
            targetNode = this.target.querySelector(`page-break[path="${targetNode.parent}"]`);
          }
          {
            targetNode = null;
          }
   ...
   */
  elementsBetween(elem, selector = "page-break", filter, direction = "nextElementSibling") {
    ...
    switch (type) {
      case "headings":
        this.breaks.forEach(element => {
          allEl = [...allEl, ...this.elementsBetween(element, "page-break", "h1,h2,h3,h4,h5,h6")];
        });
        break;
      case "noheadings":
        this.breaks.forEach(element => {
          allEl = [...allEl, ...this.elementsBetween(element, "page-break", ":not(h1,h2,h3,h4,h5,h6)")];
        });
        break;
      case "titles":
        this.breaks.forEach(element => {
          allEl = [...allEl, ...this.elementsBetween(element, "page-break", "[data-page-break-title]")];
        });
        break;
      case "notitles":
        this.breaks.forEach(element => {
          allEl = [...allEl, ...this.elementsBetween(element, "page-break", ":not([data-page-break-title])")];
        });
        break;
      case "all":
        this.breaks.forEach(element => {
  ...
  connectedCallback() {
    globalThis.addEventListener("page-break-registration", this.registerPageBreak.bind(this), {
  ...
  registerPageBreak(e) {
    ...
    if (!this.__lock) {
      ...
      setTimeout(() => {
        ...
        this.breaks.forEach(element => {
          this.elementsBetween(element, "page-break", "h1,h2,h3,h4,h5,h6").forEach(el => {
            let tagNumber = (el.getAttribute("data-original-level") ? new Number(el.getAttribute("data-original-level").replace("H", "")) : new Number(el.tagName.replace("H", ""))) + element.depth;
        ...
        if (this.target && this.target.children && this.target.children.length > 0) {
          ...
          for (let i = 0; i < kids.length; i++) {
            ...
            if (parents.length > 0) {
              ...
              let depth = 0;
              if (i !== 0 && el.parent && this.target.querySelector(`page-break[path="${el.parent}"]`)) {
                depth = this.target.querySelector(`page-break[path="${el.parent}"]`).depth + 1;
              }
...
globalThis.PageBreakManager.requestAvailability = () => {
  if (!globalThis.PageBreakManager.instance) {
    globalThis.PageBreakManager.instance = globalThis.document.createElement("page-break-manager");
