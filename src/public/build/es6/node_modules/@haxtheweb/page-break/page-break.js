 ...
 */
import { html, css, LitElement } from "../../lit/index.js";
...
import { DDDExtra } from "../d-d-d/lib/DDDStyles.js";
 ...
 * @element page-break
 */
export class PageBreak extends IntersectionObserverMixin(I18NMixin(SchemaBehaviors(LitElement))) {
  static get tag() {
    return "page-break";
  }
  constructor() {
    ...
    this.registerLocalization({
      context: this,
      localesPath: new URL("./locales/page-break.es.json", import.meta.url).href + "/../",
      locales: ["es"]
    });
  ...
  connectedCallback() {
    ...
    globalThis.dispatchEvent(new CustomEvent("page-break-registration", {
      composed: true,
      bubbles: true,
      cancelable: true,
      detail: {
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      ...
      if (propName === "schemaResourceID" && this.itemId == null && oldValue !== undefined) {
      ...
      if (this._ceMenu && ["locked", "parent", "published"].includes(propName)) {
      ...
      if (this.breakType === "node" && this.target) {
        ...
        if (!this._haxState && propName === "depth" && this.depth >= 0) {
          pageBreakManager.elementsBetween(this, "page-break", "h1,h2,h3,h4,h5,h6").forEach(el => {
            let tagNumber = (el.getAttribute("data-original-level") ? new Number(el.getAttribute("data-original-level").replace("H", "")) : new Number(el.tagName.replace("H", ""))) + this.depth;
          ...
          if (this._haxState) {
            pageBreakManager.elementsBetween(this, "page-break", "h1,h2,h3,h4,h5,h6").forEach(el => {
              if (el.getAttribute("data-original-level")) {
                let tagNumber = new Number(el.getAttribute("data-original-level").replace("H", ""));
                const newH = globalThis.document.createElement(`h${tagNumber}`);
                for (var i = 0, l = el.attributes.length; i < l; ++i) {
                  newH.setAttribute(el.attributes.item(i).nodeName, el.attributes.item(i).nodeValue);
                }
  ...
  render() {
    return html`
      ...
      ${this.locked ? html`<simple-icon-button-lite
  ...
  haxsetupActiveElementForm(props) {
    if (globalThis.HAXCMS) {
      const itemManifest = globalThis.HAXCMS.requestAvailability().store.getManifestItems(true);
      ...
      props.settings.developer.forEach((attr, index) => {
        // pull theme list from the registry
        if (attr.property === "developerTheme" && globalThis.appSettings && globalThis.appSettings.themes) {
          var themes = [{
            text: "",
            value: null
          }];
