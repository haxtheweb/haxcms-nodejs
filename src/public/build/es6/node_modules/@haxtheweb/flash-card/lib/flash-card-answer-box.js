...
export class FlashCardAnswerBox extends I18NMixin(SimpleColors) {
  ...
  constructor() {
    ...
    this.registerLocalization({
      context: this,
      localesPath: new URL("../locales/flash-card-answer-box.es.json", import.meta.url).href + "/../",
      locales: ["es", "fr", "ja"]
    });
  }
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      if (propName === "status" && oldValue !== undefined) {
        this.dispatchEvent(new CustomEvent("flash-card-status-change", {
          detail: this.status,
          bubbles: true
        }));
      }
  ...
  checkUserAnswer() {
    ...
    const comparison = this.shadowRoot.querySelector(`[name="${side}"]`).assignedNodes({
      flatten: true
    })[0].querySelector(`[name="${side}"]`).assignedNodes({
      flatten: true
    })[0].innerText;
  ...
  render() {
    return html`
      <div>
        ...
        ${this.showResult || this.correct ? html`<p class="answer-message">
      ...
      <div class="answer-section">
        ...
        />
        ${this.status === "pending" ? html`<button
              id="check"
              ?disabled="${this.userAnswer === ""}"
              @click="${this.checkUserAnswer}"
            >
              ${this.t.checkAnswer}
            </button>` : html`<span id="status-message"
              ><simple-icon-lite
                id="status-icon"
                icon="${this.icon}"
              ></simple-icon-lite
