...
class I18NManager extends LitElement {
   ...
   */
  get documentLang() {
    return globalThis.document.body.getAttribute("xml:lang") || globalThis.document.body.getAttribute("lang") || globalThis.document.documentElement.getAttribute("xml:lang") || globalThis.document.documentElement.getAttribute("lang") || globalThis.navigator.language || FALLBACK_LANG;
  }
  /**
   * Set document direction from these common sources
   */
  get documentDir() {
    return globalThis.document.body.getAttribute("xml:dir") || globalThis.document.body.getAttribute("dir") || globalThis.document.documentElement.getAttribute("xml:dir") || globalThis.document.documentElement.getAttribute("dir") || FALLBACK_DIR;
  }
  /**
   * Life cycle
   */
  connectedCallback() {
    this.__ready = true;
    globalThis.addEventListener("i18n-manager-register-element", this.registerLocalizationEvent.bind(this), {
      signal: this.windowControllers.signal
    });
    globalThis.addEventListener("languagechange", this.changeLanguageEvent.bind(this), {
      signal: this.windowControllers.signal
    });
  }
  /**
  ...
  registerLocalization(detail) {
    ...
    if (!detail.context && this.elements.filter(e => {
      return e.namespace === detail.namespace;
    }).length === 0 || this.elements.filter(e => {
  ...
  async loadNamespaceFile(ns, lang = this.lang) {
    ...
    if (nsMatch && nsMatch.length === 1) {
      ...
      if (!this.fetchTargets[fetchTarget]) {
        ...
        this.fetchTargets[fetchTarget] = await fetch(fetchTarget).then(response => {
