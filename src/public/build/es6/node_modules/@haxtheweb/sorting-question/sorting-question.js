...
import { html, css, nothing } from "../../lit/index.js";
import { QuestionElement } from "../multiple-choice/lib/QuestionElement.js";
...
export class SortingQuestion extends QuestionElement {
  ...
  constructor() {
    ...
    this.registerLocalization({
      context: this,
      localesPath: new URL("../locales/sorting-question.es.json", import.meta.url).href + "/../",
      locales: ["he", "ja", "es"]
    });
  }
  checkAnswerCallback() {
  ...
  renderInteraction() {
    return html`
      <fieldset class="options">
        ${this.displayedAnswers.map(answer => html`<sorting-option ?disabled="${this.disabled}"
              >${answer.image ? html`<img src="${answer.image}" alt="${answer.alt}" />` : nothing}${answer.label}</sorting-option
            >`)}
      </fieldset>
    `;
  }
  ...
  renderFeedback() {
    return html` ${!this.edit ? html`
          ${this.showAnswer && this.numberCorrect !== this.answers.length ? html` <p class="feedback">
                  ${this.t.numCorrectLeft} ${this.numberCorrect} out of
                  ${this.answers.length} ${this.t.numCorrectRight}
                </p>
                ${this.querySelector && this.querySelector('[slot="feedbackIncorrect"]') ? html`<slot name="feedbackIncorrect"></slot>` : ``}` : ``}
          ${this.showAnswer && this.numberCorrect === this.answers.length ? html` <p class="feedback">${this.correctText}</p>
                ${this.querySelector && this.querySelector('[slot="feedbackCorrect"]') ? html`<slot name="feedbackCorrect"></slot>` : ``}` : ``}
          ${this.querySelector && this.querySelector('[slot="hint"]') && this.showAnswer && this.numberCorrect !== this.answers.length ? html`
                <h4>Need a hint?</h4>
                <div>
                  <slot name="hint"></slot>
                </div>
              ` : ``}
          ${this.querySelector && this.querySelector('[slot="evidence"]') && this.showAnswer && this.numberCorrect === this.answers.length ? html`
                <h4>Evidence</h4>
                <div>
