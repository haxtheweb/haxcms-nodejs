...
import { displayBehaviors, editBehaviors, editableTableStyles } from "./editable-table-behaviors.js";
import "../../rich-text-editor/rich-text-editor.js";
import "../../rich-text-editor/lib/toolbars/rich-text-editor-toolbar-mini.js";
import "../../simple-toolbar/simple-toolbar.js";
...
import "../../simple-toolbar/lib/simple-toolbar-button-group.js";
import { ReplaceWithPolyfill } from "../../utils/utils.js";
...
class EditableTableEdit extends editBehaviors(DDD) {
  static get styles() {
    return [...(super.styles || []), ...editableTableStyles, css`
        :host {
  ...
  editableRow(rowIndex, rowData) {
    return html`
      <tr
        class="tr ${rowIndex == 0 && this.columnHeader ? "thead-tr" : rowIndex == this.data.length - 1 && this.footer ? "tfoot-tr" : "tbody-tr"}"
      >
        <th scope="row">${this.rowMenuButton(rowIndex)}</th>
        ${(rowData || []).map((cellData, colIndex) => this.editableCell(rowIndex, colIndex, cellData))}
      </tr>
    `;
  }
  ...
  editableCell(rowIndex, colIndex, contents) {
    return html`
      <td
        class="${colIndex === 0 && this.rowHeader || rowIndex == 0 && this.columnHeader ? "th" : "td"} th-or-td"
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      if (propName === "disabled" || propName === "hidden") this.disableEditing();
    });
  }
  disableEditing() {
    this.shadowRoot.querySelectorAll("rich-text-editor-toolbar-mini").forEach(editor => {
  ...
  _onCellValueChange(e, row, col) {
    let val = !this.shadowRoot || !this.shadowRoot.querySelector(`#cell-${row}-${col}`) ? undefined : this.shadowRoot.querySelector(`#cell-${row}-${col}`).innerHTML;
   ...
   */
  _captionChanged() {
    let val = !this.shadowRoot || !this.shadowRoot.querySelector(`#caption`) ? undefined : this.shadowRoot.querySelector(`#caption`).innerHTML;
    this.caption = val;
