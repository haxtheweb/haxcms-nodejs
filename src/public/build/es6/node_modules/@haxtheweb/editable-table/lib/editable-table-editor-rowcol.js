...
import { cellBehaviors, editableTableCellStyles } from "./editable-table-behaviors.js";
import "../../a11y-menu-button/lib/a11y-menu-button-item.js";
import { SimpleToolbarMenuBehaviors } from "../../simple-toolbar/lib/simple-toolbar-menu.js";
 ...
 */
class EditableTableEditorRowcol extends SimpleToolbarMenuBehaviors(cellBehaviors(DDD)) {
  static get styles() {
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      if (propName === "index") {
        this.controls = this.row ? `cell-0-${this.index}` : `cell-${this.index}-0`;
      }
      if (propName === "index" || propName === "row") {
        this.label = this.row ? this._getLabel(this.index, true) : this._getLabel(this.index, false);
      }
      if (propName === "row") {
  ...
  _getItem(deleteItem = false, after = false) {
    let label = `${deleteItem ? "Delete " : "Insert "}${this.type}${deleteItem ? "" : after ? " After " : " Before "}`;
    return html` <simple-toolbar-menu-item slot="menuitem">
      <simple-toolbar-button
        ...
        @button-toggled="${deleteItem ? this._onDelete : after ? this._onInsertAfter : this._onInsertBefore}"
      >
      </simple-toolbar-button>
