 ...
 * @type {array}
 */
export const editableTableStyles = [css`
    :host {
      display: block;
      width: 100%;
      max-width: 100%;
 ...
 * @type {array}
 */
export const editableTableDisplayStyles = [...editableTableStyles, css`
    caption {
  ...
  return class extends SuperClass {
    ...
    updated(changedProperties) {
      ...
      changedProperties.forEach((oldValue, propName) => {
        ...
        if (propName === "columnStriped" && this.columnStriped) this.striped = false;
     ...
     */
    focus() {
      let query = "simple-toolbar-button,rich-text-editor,rich-text-editor-rowcol,editable-table-filter,editable-table-sort",
    ...
    download() {
      let a = globalThis.document.createElement("a"),
        title = this.downloadable && this.caption.trim() != "" ? `Table as CSV` : `${this.caption} CSV`,
     ...
     */
    print() {
      let table = !this.shadowRoot ? false : this.shadowRoot.querySelector("table"),
        print = !table ? false : globalThis.open("", "", "left=0,top=0,width=552,height=477,toolbar=0,scrollbars=0,status=0");
      if (print) {
        print.document.head.innerHTML += `<style>
          ${editableTableDisplayStyles.map(s => s.cssText.replace(/:host/g, "table")).join(" ")}
          table {
    ...
    fetchData() {
      if (this.dataCsv && this.dataCsv !== "") {
        // typical use-case
        fetch(this.dataCsv).then(response => response.text()).then(data => {
          this.csvData = data;
    ...
    getTableCSV() {
      return this.data.map(row => {
        return row.map(cell => {
          ...
          return this._isNumericCell(cell) ? cell.replace(/,/g, "") : `\"${cell.replace(/"/g, '""')}\"`;
        }).join(",");
      }).join("\n");
    }
    ...
    getTableHTML(addStyleClasses = false, asNode = false) {
      ...
      let getTR = (tr, open = "td", close = "td", type = "tbody") => {
        ...
        return `\n\t\t<tr${!addStyleClasses ? "" : ` class="${type}-tr tr"`}>${th.map(cell => {
          return `\n\t\t\t<th scope="row"${!addStyleClasses ? "" : ` class="${close} th-or-td"`}>${this._replaceBlankCell(cell)}</th>`;
        }).join("")}${td.map(cell => {
          return `\n\t\t\t<${open}${!addStyleClasses ? "" : ` class="${close} th-or-td"`}>${this._replaceBlankCell(cell)}</${close}>`;
        }).join("")}\n\t\t</tr>`;
      };
      if (this.thead) {
    ...
    importHTML(table) {
      ...
      spans.forEach(span => {
        if (span.cols) for (let i = 1; i < span.cols; i++) {
          data[span.row].splice(span.col + 1, 0, "&nbsp;");
        }
        if (span.rows) for (let i = 1; i < span.rows; i++) data[span.row + 1].splice(span.col, 0, "&nbsp;");
      });
      ...
      Object.keys(displayProperties).forEach(key => {
        if (table.matches(`.${displayProperties[key].attribute || key}`)) this[key] = true;
      });
    }
     ...
     * @param {event} event
     */
    _dataChanged(newValue, oldValue) {} /*if (
                                          this.__ready &&
