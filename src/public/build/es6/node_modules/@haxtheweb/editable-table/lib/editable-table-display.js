...
import { displayBehaviors, editableTableDisplayStyles } from "./editable-table-behaviors.js";
 ...
 * @extends LitElement
 */
class EditableTableDisplay extends displayBehaviors(ResponsiveUtilityBehaviors(DDD)) {
  static get styles() {
    return [...(super.styles || []), ...editableTableDisplayStyles, css`
        :host([hidden]),
        :host([disabled]) {
      ...
      >
        ...
        <thead ?hidden="${!this.columnHeader}" class="thead" part="thead">
          <tr class="tr thead-tr" part="tr">
            ${(this.thead[0] || []).map((th, index) => html`
                <th
                  class="th th-or-td"
                  cell-index="${index}"
                ...
                >
                  ${!this.sort ? this.getHTML(this._replaceBlankCell(th)) : html`
                        <editable-table-sort
                          column-index="${index}"
                          sort-column="${this.sortColumn}"
                          sort-mode="${this.sortColumn === index ? this.sortMode : "none"}"
                        >
                          ${this.getHTML(this._replaceBlankCell(th))}
                        </editable-table-sort>
                      `}
                  ${!this.disableResponsive && index > 0 && this.selected == index ? html` <simple-picker
        ...
        <tbody class="tbody" part="tbody">
          ${this.sortedTbody.map(tr => this._isRowFiltered(tr) ? "" : this._tbodyTr(tr))}
        </tbody>
        ${!this.footer ? "" : html`
  ...
  updated(changedProperties) {
    ...
    changedProperties.forEach((oldValue, propName) => {
      if (propName === "data") this.disabled = !this.data || this.data.length < 1 || this.data[0].length < 1;
      if (["disabled", "hidden"].includes(propName) && !this.hidden && !this.disabled) {
        this.toggleFilter();
      }
    });
  }
  ...
  _tbodyTr(row = [], noFilter = false, tfoot = false) {
    return html`
      <tr class="tr ${tfoot ? "tfoot-tr" : "tbody-tr"}">
        ${row.map((cell, index) => this._isRowHeader(this.rowHeader, index) ? this._tbodyTh(cell, index) : this._tbodyTd(cell, index, noFilter))}
      </tr>
    `;
  }
  /**
    ...
    >
      ${this.filter ? html`
            <editable-table-filter
              ...
              ?toggled="${this._isCellFiltered(index, this.filterColumn, this.filtered)}"
            >
              ${this.getHTML(this._replaceBlankCell(cell))}
            </editable-table-filter>
          ` : !noFilter ? html`<span class="cell"
