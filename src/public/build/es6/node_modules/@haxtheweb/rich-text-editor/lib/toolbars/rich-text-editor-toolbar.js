...
import { SimpleToolbarBehaviors } from "../../../simple-toolbar/simple-toolbar.js";
import { SimpleToolbarButtonBehaviors } from "../../../simple-toolbar/lib/simple-toolbar-button.js";
import { RichTextStyles, RichTextToolbarStyles } from "../buttons/rich-text-editor-button.js";
import { RichTextEditorRangeBehaviors } from "../singletons/rich-text-editor-range-behaviors.js";
...
import "../buttons/rich-text-editor-emoji-picker.js";
...
import "../buttons/rich-text-editor-unlink.js";
...
const RichTextEditorToolbarBehaviors = function (SuperClass) {
  return class extends RichTextEditorRangeBehaviors(SimpleToolbarBehaviors(SuperClass)) {
    /**
    ...
    static get baseStyles() {
      return [...super.baseStyles, ...RichTextStyles, css`
          :host {
            border: var(--rich-text-editor-border-width, 1px) solid
              var(--rich-text-editor-border-color, #ddd);
    ...
    constructor() {
      ...
      this.__toolbar = this;
      globalThis.document.addEventListener(shadow.eventName, this._handleTargetSelection.bind(this.__toolbar), {
    ...
    disconnectedCallback() {
      ...
      globalThis.RichTextEditorToolbars = globalThis.RichTextEditorToolbars.filter(toolbar => toolbar !== this);
    ...
    updated(changedProperties) {
      ...
      changedProperties.forEach((oldValue, propName) => {
        ...
        if (["editor", "show", "range"].includes(propName)) this.hidden = this.disconnected;
        if (["breadcrumbs", "sticky"].includes(propName) && !!this.breadcrumbs) this.breadcrumbs.sticky = this.sticky;
      });
    }
    ...
    deregisterButton(button) {
      ...
      (button.tagsArray || []).forEach(tag => delete this.clickableElements[tag]);
    }
    /**
     * registers button when appended to toolbar
     *
    ...
    _addBreadcrumbs() {
      if (!this.breadcrumbs) {
        this.breadcrumbs = globalThis.document.createElement("rich-text-editor-breadcrumbs");
      ...
      if (this.isRangeInScope) {
        let nodes = [],
          getParentNode = node => {
            ...
            if (node.parentNode && node.parentNode !== this.target) getParentNode(node.parentNode);
          };
    ...
    enableEditing(target = this.editor) {
      ...
      if (!!target && !target.hidden && !target.disabled) {
        ...
        Object.keys(handlers).forEach(handler => target.removeEventListener(handler, handlers[handler]));
    ...
    setTarget(target) {
      ...
      if (!!target) {
        ...
        if (oldTarget !== target) {
          ...
          Object.keys(handlers).forEach(handler => target.addEventListener(handler, handlers[handler]));
          this.getRoot(target).onselectionchange = e => {
            if (!this.__promptOpen) this.updateRange(target, this.getRange());
          };
          this.target = target;
...
/**
  ...
  */
class RichTextEditorToolbar extends RichTextEditorToolbarBehaviors(LitElement) {}
