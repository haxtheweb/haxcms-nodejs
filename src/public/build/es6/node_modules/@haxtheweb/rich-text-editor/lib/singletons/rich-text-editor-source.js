...
class RichTextEditorSource extends LitElement {
  ...
  toggle(toolbar) {
    let code = this.shadowRoot ? this.shadowRoot.querySelector("#source") : undefined;
    if (!code || !toolbar || !toolbar.target || this.__toolbar && this.__toolbar === toolbar) {
      [...document.querySelectorAll("[data-rich-text-editor-view-source-mode]")].forEach(node => node.removeAttribute("data-rich-text-editor-view-source-mode"));
  ...
  _handleSourceChange(e) {
    if (!!this.__toolbar && !!this.__target && !this.__needsUpdate) {
      this.__needsUpdate = !!e.detail && !!e.detail.value ? this.__toolbar.htmlMatchesTarget(`${e.detail.value}`) : true;
      let update = () => {
...
globalThis.RichTextEditorSource.requestAvailability = () => {
  if (!globalThis.RichTextEditorSource.instance) {
    globalThis.RichTextEditorSource.instance = globalThis.document.createElement("rich-text-editor-source");
