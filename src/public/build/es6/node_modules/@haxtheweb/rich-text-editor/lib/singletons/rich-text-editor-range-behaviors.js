...
export const RichTextEditorRangeBehaviors = function (SuperClass) {
  return class extends SuperClass {
    ...
    getSelection() {
      return this.__toolbar ? this.__toolbar.getSelection() : globalThis.getSelection();
    }
    ...
    debugRange(range = this.getRange()) {
      let contents = !range || range.collapsed || !range.cloneContents ? false : range.cloneContents();
      return {
        range: range,
        contents: contents,
     ...
     * @returns
     */
    commandToggledForRange(range = this.range || this.getRange(), command = this.command, commandVal = this.commandVal) {
      let query = !!range && !!command ? globalThis.document.queryCommandState(this.command) : false,
     ...
     * @returns {node}
     */
    rangeOrMatchingAncestor(cssQuery = this.tagsList || "", range = this.range || this.getRange()) {
     ...
     */
    initViewSource() {
      this.__source = this.__source || globalThis.RichTextEditorSource.requestAvailability();
    }
    /**
    ...
    _handleCommand(command, commandVal, range = this.getRange()) {
      ...
      if (this.validCommands.includes(command)) {
        ...
        if (command == "wrapRange" && !!commandVal) {
          ...
          let toggled = this.commandToggledForRange(range, "wrapRange", commandVal),
        ...
        if (this.__highlight.parentNode === globalThis.document.body) this.__highlight.wrap(range);
        this.__highlight.unwrap(range);
